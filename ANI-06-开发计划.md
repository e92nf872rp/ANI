...
> 最后更新：2026-06-19（Sprint 12 / Core「Services 支撑 Handler」已完成 A/B1/B2/B3 全部 19 个 Core handler + 2 个 422 的 Tier1 local profile 收口；当前进入 Sprint 13 / Core real provider 与 live gate 收敛启动；本仓库只推进 ANI Core，不开发 Services/RAG/Console/BOSS/frontends；Sprint 11 / Core Real Deployment Validation 与 Rook-Ceph 正式部署结果继续作为历史回归边界保留；Sprint 13 未跑通 live gate 前，不得把 Sprint 12 local profile 能力标记为 real-provider、runtime ready 或 production ready）
...
当前重心：Sprint 13 / Core real provider 与 live gate 收敛（2026-06-19 起）；Sprint 12 已完成 A/B1/B2/B3 全部 19 个已声明未实现 Core handler + 2 个 422 的 Tier1 local profile，并完成文档闭环；Sprint 13 必须从这些既有代码边界接入真实 provider 与固定 live gate：B1 边界为 `pkg/ports/instance_observability.go`、`pkg/ports/gpu_inventory.go`、`pkg/ports/sandbox_template_catalog.go`、`pkg/adapters/runtime/local_*` 与 Gateway handler；B2 边界为 `pkg/ports/network_resources.go`、`pkg/ports/storage_resources.go`、`pkg/ports/k8s_clusters.go`、`pkg/adapters/runtime/local_*` 与 Gateway handler；B3 边界为 `pkg/ports/storage_resources.go`、`pkg/ports/object_store.go`、`pkg/ports/vector_store.go`、`pkg/adapters/runtime/storage_service.go`、`pkg/adapters/runtime/vector_store_service.go` 与 Gateway handler；未执行对应 live gate 前，不代表 real-provider、runtime ready 或 production ready；详见 repo/CURRENT-SPRINT.md
当前执行入口：Sprint 13 真实 provider / live gate 计划见 repo/development-records/sprint13-real-provider-readiness-plan.md；S01 网络路由 Kube-OVN、S02 K8s workloads vCluster、S03 storage Rook-Ceph 与 S04 GPU NVIDIA device-plugin/DCGM 已完成 A 轨 code+contract ready（LIVE PENDING），执行时必须先声明真实组件与版本、live gate 命令、evidence 输出路径和失败边界
最近里程碑（详情见 repo/CURRENT-SPRINT.md 已完成切片）：
  ...
  2026-06-19  SPRINT13-STORAGE-ROOK-CEPH-A-TRACK：storage Rook-Ceph S03 A 轨完成；`KubernetesStorageRenderer` 增加 CSI `VolumeSnapshot` 与 mount-target `Service` contract manifest，provider dry-run / REST client fake 单测覆盖，新增 `validate-storage-live-gate`；未执行真实 live gate，仍为 Tier1 local profile / LIVE PENDING [A-track]
  2026-06-19  SPRINT13-K8S-WORKLOADS-VCLUSTER-A-TRACK：K8s workloads vCluster S02 A 轨完成；real-provider cluster 通过既有 proxy target 只读调用 Kubernetes API workload list，fake 单测覆盖 Deployment 解析，`validate-vcluster-live-gate` 增加 `core-workloads-list` contract；未执行真实 live gate，仍为 Tier1 local profile / LIVE PENDING [A-track]
  2026-06-19  SPRINT13-NETROUTE-KUBEOVN-A-TRACK：网络路由 Kube-OVN S01 A 轨完成；新增 adapter-local route→Kube-OVN `Vpc.spec.staticRoutes` renderer，provider dry-run fake 单测覆盖 route manifest，`validate-kubeovn-network-live-gate` 增加 `kubeovn-route-created` contract；未执行真实 live gate，仍为 Tier1 local profile / LIVE PENDING [A-track]
...
| Sprint 13 ⭐ | 进行中 | 2026-06-19 起 | 真实 provider / live gate 收敛；从 Sprint 12 已建立的 ports/adapters/router 边界接入 K8s/kubelet/Prometheus/DCGM/Kata/Kube-OVN/Rook-Ceph/MinIO/向量后端等真实组件；S01 网络路由 Kube-OVN、S02 K8s workloads vCluster、S03 storage Rook-Ceph 与 S04 GPU NVIDIA device-plugin/DCGM 已到 code+contract ready / LIVE PENDING（`SPRINT13-GPU-INVENTORY-DCGM-A-TRACK` / `validate-gpu-inventory-live-gate`）；计划见 `repo/development-records/sprint13-real-provider-readiness-plan.md`，未执行 live gate 前不得标 runtime/production ready |
