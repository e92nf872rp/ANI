...
- 实例交互式终端 WebSocket：INSTANCE-EXEC-WS-A；`POST /api/v1/instances/{id}/exec` 返回短期 token 与内嵌 token 的浏览器直连 `ws_url`，新增 `GET /api/v1/instances/{id}/exec/{session}` WebSocket 握手入口；RBAC 固定 `scope:instances:exec`，数据帧为原始 stdin/stdout 透传，resize 为 JSON 控制帧；local/logic verified，不修改 Console 前端，不放宽鉴权，真实 Kubernetes Pod exec streaming 后续在 adapter 边界补齐。
