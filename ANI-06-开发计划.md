...
租户计费结算接口：TENANT-BILLING-A（2026-09-08，local verified，Services PR）——BOSS 租户计费结算 5 端点 `/api/v1/svc/billing/*`（总览/CSV 导出/生成账单/账单动作 settle·credit/调账）按方案 v0.3 落地：Services 层归属（契约 `services/v1.yaml` + x-ani-authz boundary=platform，实现 tenant-service gRPC，网关 mixed handler 转发）；生成即出账（无 draft）、一期一单幂等、overdue 读取时动态计算不落库、余额读取推导、单价只读 `billing_pricing`（种子 ON CONFLICT DO NOTHING，代码零价格字面量）、用量仅经 Core SDK 调 `/metering/usage/platform` 不直查 Core 库、仅 token_total 计费。本地集成实测 21 用例通过（测试环境占用未部署 K8s，不标 live/runtime ready）；真环境 RBAC 回归与成本对账留待后续批次。批次详情 `repo/development-records/TENANT-BILLING-A.md`。
租户计费操作历史：TENANT-BILLING-B（2026-09-08，local verified，Services PR）——在 TENANT-BILLING-A 基础上追加第 6 个只读端点 `GET /api/v1/svc/billing/operations`（BOSS 租户计费抽屉「操作历史」Tab 数据源，tenant_id/limit/offset，created_at 倒序，limit 上限 200 钳制，无足迹租户空列表不 404）；新表 `billing_operation_logs` 迁移（已在测试环境 PG 落表），生成/结清/冲抵/调账 4 个写点与业务写**同事务**落流水（业务写回滚则流水不落；幂等重放与 409 冲突不产生流水），operator 透传 token user_id；契约先行 + pb/Services SDK 四语言/docs/api 重生成幂等零漂移；顺带修复 `services/v1.yaml` 裸 `no` 被 YAML 1.1 解析为 False 的契约缺陷（加引号，JSON 字段名不变）。单测 svc +4 / gateway +2 全 PASS；混合联调 13 用例（e30–e43，本地进程 × 环境 PG/Redis/auth-service）全部通过；已合并 main（#148/#150/#152）并全量回归。**同批次追加账单软删除 `DELETE /billing/invoices/{invoiceId}`（第 7 个端点）**：仅 issued 可删（终态 409、不存在/已删 404 幂等无害）、`deleted_at` 软删行保留可审计、`(tenant_id, period)` 改部分唯一索引释放唯一键（同账期可重新出账、单号 seq 不复用）、删除与 `invoice.deleted` 流水同事务落库；迁移 `20260908_002_billing_invoice_soft_delete.sql` 已应用测试环境 PG 并复核；顺带修复软删行对 settle/credit CAS 可见的边界缺陷（已删账单再结清 404，svc 断言锁定）；单测全 PASS，混合联调 6/6（e44–e48）。未部署 K8s，不标 live/runtime ready。批次详情 `repo/development-records/TENANT-BILLING-B.md`。当前执行入口：repo/CURRENT-SPRINT.md
