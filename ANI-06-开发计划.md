...
- VM Console / noVNC WebSocket：INSTANCE-CONSOLE-VNC-WS-A；修复创建 session 时对 KubeVirt `/vnc` 的普通 HTTP GET 探测；`POST /api/v1/instances/{id}/console` 返回内嵌 token 的 `connect_url`，新增 `GET /api/v1/instances/{id}/console/{session}`；Gateway 以 `plain.kubevirt.io` 转发到 KubeVirt，供浏览器 noVNC 直连；local/logic verified，不改 Services/前端，不标 production ready。
...
- ISO VM CD-ROM detach：ISO-CDI-VM-CDROM-DETACH-A；安装完成后若不移除 `bootOrder=1` 的 ISO CD-ROM，VM 重启会再次进入安装引导；`detach_volume volume_id=iso` 现在会在 VM `cdrom` 场景调用 Kubernetes lifecycle provider，GET KubeVirt VM 后 merge-patch 删除 `iso` disk/volume；ISO VM 渲染改为 rootdisk 持有 `bootOrder=1`、ISO CD-ROM 不持有 bootOrder，使后续重启进入 root disk；local/logic verified，live 待验证，不标 production ready。
