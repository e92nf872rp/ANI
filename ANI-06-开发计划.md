...
> 最后更新：2026-06-20（Sprint 12 / Core「Services 支撑 Handler」已完成 A/B1/B2/B3 全部 19 个 Core handler + 2 个 422 的 Tier1 local profile 收口；当前进入 Sprint 13 / Core real provider 与 live gate 收敛；S01 网络路由 Kube-OVN、S02 K8s workloads vCluster、S03 storage Rook-Ceph 与 S04 GPU NVIDIA device-plugin/DCGM 已重新通过 production-shaped live gate 并归档 `production_shape.status=passed` evidence；S01 强制经 ANI Gateway create/list 后再观测 Kube-OVN 底层对象，S02-S04 分别固定 metadata target TLS、in-cluster RBAC、cluster Service metrics proof_items 与 workload/storage/GPU+DCGM 正向业务证据；SPRINT13-AUTH-DEX-PRODUCTION-GATE（Auth/Dex production gate）已通过真实集群生产形态验证，production-shaped Gateway 使用 `ANI_AUTH_MODE=auth_service`，并经 `validate-auth-dex-production-gate` 固定 anonymous 401、OIDC begin/complete 200、protected API bearer 200 与 refresh 200；S01-S04 的 Auth/Dex production ready 阻断已解除，但该结论仍不是 full platform production ready；S05-S07 B 轨可以继续，且已完成 A 轨 code+contract ready / LIVE PENDING；本仓库只推进 ANI Core，不开发 Services/RAG/Console/BOSS/frontends；Sprint 11 / Core Real Deployment Validation 与 Rook-Ceph 正式部署结果继续作为历史回归边界保留；S01-S04 production-shaped acceptance passed 不等于 full platform production ready，Sprint 13 未跑通对应 production-shaped live gate 前不得把 S05-S07 标记为 runtime ready 或 production ready）
...
当前阶段：Phase 1 / Sprint 13 / Core real provider 与 live gate 收敛启动；Sprint 12 / Core「Services 支撑 Handler」已闭环并转为历史回归门禁
...
当前重心：Sprint 13 / Core real provider 与 live gate 收敛（2026-06-19 起）；Sprint 12 已完成 A/B1/B2/B3 全部 19 个已声明未实现 Core handler + 2 个 422 的 Tier1 local profile，并完成文档闭环；Sprint 13 必须从这些既有代码边界接入真实 provider 与固定 live gate：B1 边界为 `pkg/ports/instance_observability.go`、`pkg/ports/gpu_inventory.go`、`pkg/ports/sandbox_template_catalog.go`、`pkg/adapters/runtime/local_*` 与 Gateway handler；S04 GPU 已补齐 `GPU_INVENTORY_PROVIDER=kubernetes_rest` 接线路径，并通过 DCGM exporter + Core GPU inventory production-shaped live gate；B2 边界为 `pkg/ports/network_resources.go`、`pkg/ports/storage_resources.go`、`pkg/ports/k8s_clusters.go`、`pkg/adapters/runtime/local_*` 与 Gateway handler，S01 网络路由 Kube-OVN 已完成 Gateway VPC/Subnet/Route provider create/list 与底层 Kube-OVN 观测，S02 K8s workloads vCluster 已通过 metadata target TLS workload list live gate，S03 storage Rook-Ceph 已通过 production-shaped snapshot/mount-target live gate；S01-S04 四份 evidence 均为 `production_shape.status=passed` 并受 `validate-sprint13-b-track-production-shape` 约束；B3 边界为 `pkg/ports/storage_resources.go`、`pkg/ports/object_store.go`、`pkg/ports/vector_store.go`、`pkg/adapters/runtime/storage_service.go`、`pkg/adapters/runtime/vector_store_service.go` 与 Gateway handler；SPRINT13-AUTH-DEX-PRODUCTION-GATE（Auth/Dex production gate）已通过，Gateway production-shaped 部署为 `ANI_AUTH_MODE=auth_service` 并使用集群内 auth-service；S01-S04 的 Auth/Dex production ready 阻断已解除，但 full platform production ready 仍需正式镜像发布/升级、长期 SLA/soak、备份/恢复和故障注入等 release gate；S05-S07 B 轨可以继续，但未执行对应 production-shaped live gate 前，S05-S07 不代表 real-provider、runtime ready 或 production ready；详见 repo/CURRENT-SPRINT.md
当前执行入口：Sprint 13 真实 provider / live gate 计划见 repo/development-records/sprint13-real-provider-readiness-plan.md；S01 网络路由 Kube-OVN 已通过 B 轨真实 live gate（evidence：repo/development-records/live-evidence/sprint13-netroute-kubeovn-live-evidence.json；result：repo/development-records/sprint13-netroute-kubeovn-live-result.md），S02 K8s workloads vCluster 已通过 B 轨真实 workload list live gate（evidence：repo/development-records/live-evidence/sprint13-k8s-workloads-vcluster-live-evidence.json；result：repo/development-records/sprint13-k8s-workloads-vcluster-live-result.md），S03 storage Rook-Ceph 已通过 B 轨真实 snapshot/mount-target live gate（evidence：repo/development-records/live-evidence/sprint13-storage-rook-ceph-live-evidence.json；result：repo/development-records/sprint13-storage-rook-ceph-live-result.md），S04 GPU NVIDIA device-plugin/DCGM 已通过 B 轨真实 GPU inventory live gate（evidence：repo/development-records/live-evidence/sprint13-gpu-inventory-dcgm-live-evidence.json；result：repo/development-records/sprint13-gpu-inventory-dcgm-live-result.md）；S05 object-store MinIO、S06 vector Milvus 与 S07 instance observability Prometheus 已完成 A 轨 code+contract ready（LIVE PENDING），B 轨执行时必须先声明真实组件与版本、live gate 命令、evidence 输出路径和失败边界
最近里程碑（详情见 repo/CURRENT-SPRINT.md 已完成切片）：
  2026-06-20  SPRINT13-AUTH-DEX-PRODUCTION-GATE：Auth/Dex production gate 通过；真实集群部署 Dex + auth-service + `ANI_AUTH_MODE=auth_service` Gateway，`validate-auth-dex-production-gate` 固定 Dex discovery/JWKS、anonymous 401、OIDC begin/complete 200、protected API bearer 200 与 refresh 200，非敏感 evidence 已归档；解除 S01-S04 Auth/Dex production ready 阻断，但不代表 full platform production ready [Production Auth]
  2026-06-20  SPRINT13-B-TRACK-PRODUCTION-SHAPED-PASSED：S01-S04 B 轨 production-shaped live gate 重新执行并通过；S01 修复 network provider 从 route-only 提升为 VPC/Subnet/SecurityGroup/LoadBalancer/Route provider pipeline，Kubernetes server-side dry-run 改为 PATCH dryRun=All，证据强制包含 Gateway create/list 字段；S02/S03/S04 证据均为 `production_shape.status=passed`；不代表 full platform production ready [Production-shaped live]
  2026-06-20  SPRINT13-B-TRACK-PRODUCTION-SHAPED-POST-REVIEW：S01-S04 B 轨 production-shaped 深度复审完成；bootstrap `Config` 与 `NewCapabilitiesWithConfig` 统一支持 in-cluster ServiceAccount Kubernetes REST provider 装配，覆盖 S01 network、S03 storage、S04 GPU inventory 与 S07 Prometheus observability；adapter 层不再隐式读取 ambient env [Production-shaped hardening]
  2026-06-20  SPRINT13-B-TRACK-PRODUCTION-SHAPED-CLOSURE：S01-S04 B 轨 production-shaped closure 完成；Kubernetes REST client 支持 in-cluster ServiceAccount token/CA，Gateway network/storage/gpu runtime 透传 in-cluster 配置，S01 route metadata 新增持久化与 migration，S02/S03/S04 live gate 增加 `--production-shaped` 模式，production profile/RBAC 固定 S01-S07 proof_items 标准 [Production-shaped closure]
  2026-06-20  SPRINT13-B-TRACK-PRODUCTION-SHAPE-REVIEW：S01-S04 B 轨生产形态边界审查完成；新增 `validate-sprint13-b-track-production-shape`，禁止把 lab kubeconfig、kubectl proxy、port-forward 或 dev gateway evidence 误标为 production ready；后续已升级为 passed evidence proof_items guard [Guard]
  2026-06-20  SPRINT13-GPU-INVENTORY-DCGM-LIVE-A：S04 GPU B 轨真实 live gate 通过；恢复 DCGM exporter Helm release `ani-dcgm-exporter`，确认 NVIDIA device-plugin `v0.19.2` 与 DCGM exporter `4.8.2` DaemonSet 均为 `3/3 ready`、三节点合计 6 GPU；`GPU_INVENTORY_PROVIDER=kubernetes_rest` 下 Core `/gpu-inventory`、`/gpu-inventory/occupancy`、Kubernetes NodeList GPU capacity 与 DCGM `DCGM_FI_DEV_GPU_UTIL` 已通过 `validate-gpu-inventory-live-gate --live`，evidence JSON 已归档；S04 可标 real-provider evidence passed，不代表 production ready 或 S05-S07 完成 [B-track live]
  2026-06-20  SPRINT13-STORAGE-ROOK-CEPH-LIVE-A：storage Rook-Ceph S03 B 轨真实 live gate 通过；安装/恢复 CSI snapshot CRDs/controller，创建默认 RBD `VolumeSnapshotClass`，`STORAGE_PROVIDER=kubernetes_rest` provider-backed storage service 完成 Core volume create、snapshot create/list、filesystem create、mount-target list 与 cleanup，evidence JSON 已归档；S03 storage snapshot/mount-target 可标 real-provider evidence passed，不代表 production ready 或 S04-S07 完成 [B-track live]
  2026-06-20  SPRINT13-K8S-WORKLOADS-VCLUSTER-LIVE-A：K8s workloads vCluster S02 B 轨真实 live gate 通过；恢复 vCluster CLI `v0.34.1`，`validate-vcluster-live-gate` 固定 Helm chart version `0.34.1` 并支持显式 `--proxy-server`；真实执行临时 Deployment create、Core proxy `/version`、Core workload list observe 与 cleanup，evidence JSON 已归档；S02 workload list provider 可标 real-provider evidence passed，不代表 production ready 或后续切片完成 [B-track live]
  2026-06-19  SPRINT13-INSTANCE-OBSERVABILITY-PROMETHEUS-A-TRACK：instance observability S07 A 轨完成；新增 `PrometheusInstanceObservability` adapter 与 `INSTANCE_OBSERVABILITY_PROVIDER=prometheus_kubernetes` 显式 bootstrap 注入路径，新增 `validate-instance-observability-live-gate` 覆盖 Prometheus readiness、Core logs/events/metrics/security-events/exec session；未执行真实 live gate，仍为 Tier1 local profile / dev_profile / LIVE PENDING [A-track]
  2026-06-20  SPRINT13-NETROUTE-KUBEOVN-LIVE-A：网络路由 Kube-OVN S01 B 轨真实 live gate 通过；`NetworkProviderRenderer.RenderRoute`、`LocalNetworkService.CreateRoute` provider 管线、bootstrap/Gateway runtime `NETWORK_PROVIDER=kubeovn_rest` 注入路径已闭合；执行 `validate_kubeovn_network_live_gate.py --live --cleanup`，真实创建/观察临时 Vpc/Subnet/route/NetworkPolicy/Service，evidence JSON 已归档且 cleanup 核验为空输出；S01 route provider 可标 real-provider evidence passed，不代表 production ready 或后续切片完成 [B-track live]
  2026-06-20  SPRINT13-NETROUTE-KUBEOVN-B-TRACK-PREP：网络路由 Kube-OVN S01 B 轨接线准备完成；`NetworkProviderRenderer.RenderRoute` 纳入 port，`LocalNetworkService.CreateRoute` 可在 `NETWORK_PROVIDER=kubeovn_rest` 时走 route renderer→dry-run→apply→observe，bootstrap/Gateway runtime 增加 provider execution proof 配置，live gate 增加 `--cleanup` 临时资源清理 [B-track prep]
  2026-06-19  SPRINT13-VECTOR-MILVUS-A-TRACK：vector document insert S06 A 轨完成；新增 `MilvusVectorStore` REST adapter 与 `VECTOR_STORE_PROVIDER=milvus` 显式 bootstrap 注入路径，新增 `validate-vector-store-live-gate` 覆盖 Milvus readiness、Core vector store create、document insert 202 与 search readiness；未执行真实 live gate，仍为 Tier1 local profile / LIVE PENDING [A-track]
  2026-06-19  SPRINT13-OBJECTSTORE-MINIO-A-TRACK：object-store bucket/upload/download S05 A 轨完成；新增 `MinIOObjectStore` S3-compatible adapter 与 `OBJECT_STORE_PROVIDER=minio` 显式 bootstrap 注入路径，新增 `validate-object-store-live-gate` 覆盖 bucket create/list、upload pre-signed URL、download pre-signed URL；未执行真实 live gate，仍为 Tier1 local profile / LIVE PENDING [A-track]
  2026-06-19  SPRINT13-GPU-INVENTORY-DCGM-A-TRACK：GPU inventory / occupancy S04 A 轨完成；新增 `KubernetesGPUInventory` 只读解析 Kubernetes NodeList 中 `nvidia.com/gpu` capacity、NVIDIA device-plugin product label、node readiness 与 nodeInfo，新增 `validate-gpu-inventory-live-gate` 覆盖 Core `/gpu-inventory`、`/gpu-inventory/occupancy` 与 DCGM readable contract；未执行真实 live gate，仍为 Tier1 local profile / LIVE PENDING [A-track]
  2026-06-19  SPRINT13-STORAGE-ROOK-CEPH-A-TRACK：storage Rook-Ceph S03 A 轨完成；`KubernetesStorageRenderer` 增加 CSI `VolumeSnapshot` 与 mount-target `Service` contract manifest，provider dry-run / REST client fake 单测覆盖，新增 `validate-storage-live-gate`；该条记录为 A-track 当时状态，后续 B 轨 live result 已通过 [A-track]
  2026-06-19  SPRINT13-K8S-WORKLOADS-VCLUSTER-A-TRACK：K8s workloads vCluster S02 A 轨完成；real-provider cluster 通过既有 proxy target 只读调用 Kubernetes API workload list，fake 单测覆盖 Deployment 解析，`validate-vcluster-live-gate` 增加 `core-workloads-list` contract；该条记录为 A-track 当时状态，后续 B 轨 live result 已通过 [A-track]
  2026-06-19  SPRINT13-NETROUTE-KUBEOVN-A-TRACK：网络路由 Kube-OVN S01 A 轨完成；新增 adapter-local route→Kube-OVN `Vpc.spec.staticRoutes` renderer，provider dry-run fake 单测覆盖 route manifest，`validate-kubeovn-network-live-gate` 增加 `kubeovn-route-created` contract；本 A 轨未执行真实 live gate，后续 B 轨 live result 已通过 [A-track]
  2026-06-19  SPRINT12-CLOSURE-A：Sprint 12 Core「Services 支撑 Handler」完成收口；A/B1/B2/B3 全部 19 个 Core handler + 2 个 422 均经 `api/openapi/v1.yaml`、`pkg/ports`、`pkg/adapters`、Gateway handler 与测试闭合；进入 Sprint 13 real provider/live gate 收敛 [Closure]
  2026-06-19  CORE-SVC-SUPPORT-OBJVEC-A：B3 对象存储 bucket/object 预签名 URL 与 vector document insert 完成；扩展 `StorageService` / `VectorStoreService` ports 与 local adapters，复用 `ports.ObjectStore.EnsureBucket` / `SignedUploadURL` / `SignedDownloadURL` 和 `ports.VectorStore.Upsert` 边界，注册 5 个 operationId；Tier1 local profile，不声明 runtime/production ready [Feature batch]
  2026-06-19  CORE-SVC-SUPPORT-NETSTORE-A：B2 网络路由、卷快照、filesystem mount-targets、K8s workloads 与 2 个 422 完成并复审收口；扩展 `NetworkService` / `StorageService` / `K8sClusterService` ports 与 local adapters，注册 6 个 operationId；`createVolumeSnapshot` 的 202 响应按全局约定返回 `AsyncTask`；Tier1 local profile，响应带 dev_profile，不声明 runtime/production ready [Feature batch]
  ...
  2026-06-05  CORE-ROOK-CEPH-FORMAL-DEPLOYMENT-A：新增 Rook-Ceph 正式部署代码包，包含 `CephCluster`、`CephBlockPool`、`ani-rbd-ssd` StorageClass 和 validator；只使用 `/dev/disk/by-id` SSD 候选盘，排除 HDD，未执行 live apply 或 OSD 认领 [Feature batch]
  2026-06-05  SPRINT11-SAFE-CLOSURE-A：Sprint 11 Core Real Deployment Validation 正式部署完成闭环建立；不是实际 v1.0.0 发布或完整 production ready [Closure]
  2026-06-05  CORE-SAFE-COMPLETION-A：新增 Sprint 11 Core safe completion profile 和 validator，固定无服务器写操作、无重启、无数据丢失风险接受、人工审批前禁止存储状态变更 [Feature batch]
  2026-06-04  CORE-REAL-DEPLOY-DOC-CONSISTENCY-A：新增 Sprint 11 Core 文档一致性 gate，校验入口文档、Makefile targets 和 development records 与真实部署验证状态一致 [Feature batch]
  2026-06-04  CORE-REAL-DEPLOY-A：新增 Sprint 11 Core real deployment validation profile，聚合 Sprint 10 release-prep、REAL-K8S-LAB profile、K8s/KubeVirt/storage 只读验证和文档一致性 gate；不执行服务器写操作 [Feature batch]
  2026-06-04  CORE-STORAGE-DISK-RISK-A：新增三台物理机数据盘风险计划，固定 `/dev/disk/by-id` 设备映射，禁止依赖 `/dev/sdX` 顺序和未审批的 wipe/format/mount/Rook-Ceph OSD 认领 [Feature batch]
  2026-06-04  SPRINT11-KICKOFF-A：入口文档切换到 Sprint 11 / Core Real Deployment Validation；只进行 ANI Core 真实服务器只读验证和风险建模 [Kickoff]
  2026-06-04  SPRINT10-CLOSURE-A：Sprint 10 Core-only artifact manifest / version policy / final readiness / CLI release metadata / doc consistency 五个 contract/local validation 批次全部完成并通过门禁；入口文档切换到 Sprint 10 completed / release-prep completed 状态；不是实际 v1.0.0 发布 [Closure]
  2026-06-04  CORE-FINAL-DOC-CONSISTENCY-A：新增 Sprint 10 Core 文档一致性 gate，校验三份入口文档、Makefile targets、development records 索引和 release boundary marker [Feature batch]
  2026-06-04  CORE-CLI-RELEASE-METADATA-A：新增 `ani --version --version-format json`，输出机器可读 Core CLI release metadata [Feature batch]
  2026-06-04  CORE-FINAL-READINESS-A：新增 Sprint 10 Core final readiness profile，聚合 Sprint 9 RC、artifact manifest、version policy、CLI build、SDK/API/doc gates [Feature batch]
  2026-06-04  CORE-VERSION-POLICY-A：新增 Core version policy，确保 `actual_release` / `release_candidate` / `production_release` / `rc_cut` 均保持 false；不是实际 v1.0.0 发布 [Feature batch]
  2026-06-04  CORE-ARTIFACT-MANIFEST-A：新增 Core artifact manifest，固定 Core OpenAPI、Core SDK metadata、CLI source、offline lock、release evidence 的 SHA256 清单 [Feature batch]
  2026-06-04  SPRINT9-CLOSURE-A：Sprint 9 Core-only RC readiness / release evidence / offline checksum / CLI version / doc consistency 五个 contract/local validation 批次全部完成并通过门禁；入口文档切换到 Sprint 9 completed / Sprint 10 prep 状态 [Closure]
  2026-06-04  CORE-RC-DOC-CONSISTENCY-A：新增 Sprint 9 Core 文档一致性 gate，校验三份入口文档、Makefile targets 和 development records 索引 [Feature batch]
  2026-06-04  CORE-CLI-VERSION-A：新增 `ani --version`，支持 `Version` / `BuildTime` 注入与本地输出；不新增 Services 命令 [Feature batch]
  2026-06-04  CORE-OFFLINE-CHECKSUM-A：将 Core offline package lock 从占位 checksum 收敛为可复算 source manifest checksum；不代表真实离线 tarball 已签名交付 [Feature batch]
  2026-06-04  CORE-RELEASE-EVIDENCE-A：新增 Core release evidence manifest，固定可复跑命令和非敏感 artifact 引用；不代表实际 RC cut 或 production release [Feature batch]
  2026-06-04  CORE-RC-GATE-A：新增 Sprint 9 Core RC readiness profile，聚合 API 兼容、架构、文档、SDK、Sprint 8 release、release evidence、offline pack、CLI version 和 doc consistency gates [Feature batch]
  2026-06-04  SPRINT8-CLOSURE-A：Sprint 8 Core-only release hardening / installer live readiness / offline package lock / CLI-B / doc consistency 五个 contract/local validation 批次全部完成并通过门禁；入口文档切换到 Sprint 8 completed / Sprint 9 prep 状态 [Closure]
  2026-06-04  CORE-DOC-CONSISTENCY-A：新增 Sprint 8 Core 文档一致性 gate，校验三份入口文档、Makefile targets 和 development records 索引 [Feature batch]
  2026-06-04  CORE-CLI-B：扩展 `cli/ani` Core CLI 只读命令覆盖 network/storage/vector/encryption/observability 资源，继续拒绝 Services 业务命令 [Feature batch]
  2026-06-04  CORE-OFFLINE-PACK-A：新增 Core offline package lock，固定 artifact/checksum/verification contract；不代表真实离线包已制作、签名或客户现场交付 [Feature batch]
  2026-06-04  CORE-INSTALLER-LIVE-A：新增 Core installer live-readiness profile，固定三种安装模式 evidence 入口；不代表真实安装或 production ready [Feature batch]
  2026-06-04  CORE-HARDEN-A：新增 Core release hardening profile，覆盖 Core API 兼容、架构、文档、SDK、CLI、installer/offline gates [Feature batch]
  2026-06-04  SPRINT7-CLOSURE-A：Sprint 7 Core-only installer/offline/CLI/regression 四个 contract/local validation 批次全部完成并通过门禁；入口文档切换到 Sprint 7 completed / Sprint 8 prep 状态 [Closure]
  2026-06-04  CORE-REGRESSION-A：新增 Sprint 7 Core regression profile，串联 installer/offline/CLI 与历史 real path/SDK smoke gates；不新增 M1-REAL-LAB guard [Feature batch]
  2026-06-04  CORE-CLI-A：新增 `cli/ani` Core CLI 最小行为，支持 Core REST base URL/bearer token、Core 资源只读请求和 Services 资源拒绝；不代表 CLI 全资源覆盖或 production ready [Feature batch]
  2026-06-04  CORE-OFFLINE-A：新增 Core offline package manifest 和校验器，固定 Core 镜像/Helm chart/script 清单；不代表离线包已制作或可交付 [Feature batch]
  2026-06-04  CORE-INSTALLER-A：新增 Core installer baremetal/VM/existing-k8s 三种 profile 和校验器；不代表真实安装或 production ready [Feature batch]
  2026-06-04  SPRINT7-KICKOFF-A：入口文档切换到 Sprint 7 Core-only 代码开发阶段；当前仓库只推进 ANI Core installer/离线包/Core CLI/真实回归门禁，不开发 Services/RAG/Console/frontends [Kickoff]
  2026-06-03  SPRINT6-CLOSURE-A：Sprint 6 Sandbox/Observability/Metering/Registry 四个 Core API/local profile 批次全部完成并通过完整门禁；入口文档切换到 Sprint 6 completed / Sprint 7 kickoff 状态 [Closure]
  2026-06-03  M1-REGISTRY-A 镜像仓库 API local profile：Core OpenAPI 新增 registry projects/repositories/artifacts/permissions/pull-secret/scan-report/scan-result，扩展 `ImageRegistry` port，新增 local adapter、Gateway 路由、adapter/router 单元测试和 Core SDK metadata；已通过完整批次门禁；不代表 Harbor/Trivy real provider 或 production ready [Feature batch]
  2026-06-03  M1-METER-A 计量 API local profile：Core OpenAPI 补齐 `GET /metering/usage` 响应并新增 `POST /metering/token-usage`，新增 `MeteringService` port、local adapter、Gateway 路由、adapter/router 单元测试和 Core SDK metadata；已通过完整批次门禁；不代表真实 metering/billing backend 或 production ready [Feature batch]
  2026-06-03  M1-OBS-A 可观测性 API local profile：Core OpenAPI 新增 `GET /observability/query` 与 `/observability/alert-rules` CRUD，新增 `ObservabilityService` port、local adapter、Gateway 路由、adapter/router 单元测试和 Core SDK metadata；已通过完整批次门禁；不代表 Prometheus/Alertmanager real provider 或 production ready [Feature batch]
  2026-06-03  M1-SANDBOX-A Kata Containers 安全沙箱实例类型 local profile：Core OpenAPI `/instances` 新增 `sandbox`/`instance_type`/`sandbox_config`/响应摘要，新增 `SandboxRuntime` port、local adapter、Gateway 映射、adapter/router 单元测试和 Core SDK metadata；已通过完整批次门禁；不代表真实 Kata provider 或 production ready [Feature batch]
  2026-06-03  M1-K8S-LIVE-M node pool CAPK real lab result：真实执行增强后的 `validate_k8s_node_pool_live_gate.py --live`，Core node pool create/update 后 CAPK MachineDeployment/MachineSet/Machine/KubevirtMachine/VM/VMI 均达到 2 副本 Ready/Available/Running；不代表 CAPK VM 内 GPU passthrough/vGPU 完成 [真实环境]
  2026-06-02  M1-K8S-LIVE-J node pool CAPI schema hardening：修正 Cluster API `MachineDeployment` manifest 为真实 CAPI v1beta1 合法结构，GPU/规格 intent 改由 metadata 保留；不代表 node pool/GPU live gate 通过 [代码边界修正]
  2026-06-02  M1-K8S-LIVE-L node pool CAPK ref config：在默认保持 `ANIMachineTemplate` 输出兼容的前提下，支持通过 Gateway env 配置 CAPK `KubeadmConfigTemplate` / `KubevirtMachineTemplate` refs 与 machine version；不代表 CAPI/CAPK live create/scale 通过 [代码边界修正]
  2026-06-02  M1-K8S-LIVE-K GPU scheduling real lab progress：ANI1/ANI2/ANI3 三台服务器均完成 NVIDIA driver、NVIDIA Container Toolkit 和 NVIDIA device plugin 部署，三节点 `nvidia.com/gpu` allocatable、通用 GPU smoke Pod 与 ANI1 control-plane 专属 GPU smoke 已真实通过；不代表 Cluster API node pool create/scale 通过 [真实环境]
  2026-06-02  M1-NETWORK-LIVE-D Kube-OVN external LoadBalancer real lab result：增强 `validate-kubeovn-network-live-gate`，external LB helper EIP/DNAT/MASQUERADE 与三节点 HTTP 可达性通过 [真实环境]
  2026-06-02  M1-KUBEVIRT-LIVE-D KubeVirt console/VNC WebSocket session real lab result：增强 `validate-kubevirt-vm-live-gate`，console/VNC 均完成 HTTP 101、`plain.kubevirt.io` 子协议回选和流数据字节验证 [真实环境]
  2026-06-02  M1-SECRETS-LIVE-D VM Secret guest real lab result：增强 `validate-secrets-live-gate`，真实 KubeVirt VM guest 可读取 Secret volume 中的 key 文件；evidence 不含 Secret 明文 [真实环境]
  2026-06-02  M1-K8S-LIVE-I node pool/GPU real lab blocker evidence：当时真实 lab 无 Cluster API MachineDeployment API resource 且 GPU smoke Pod 实测 `Insufficient nvidia.com/gpu`；GPU 部分已由后续 `M1-K8S-LIVE-K` 解除，CAPI/CAPK create/scale 已由 `M1-K8S-LIVE-M` 解除 [历史阻塞证据]
  2026-06-02  M1-SECRETS-LIVE-C Secret real lab live result：Kubernetes Secret provider 写入、Pod env/file 可见、KubeVirt VM Secret volume manifest API 接受 [真实环境]
  2026-06-02  M1-K8S-LIVE-H vCluster upgrade real lab live result：Core provider-backed create、Helm upgrade values、升级后 vCluster `/version` 与 Core live proxy `/version` 通过 [真实环境]
  2026-06-02  M1-K8S-LIVE-G vCluster real lab live result：Helm install、kubeconfig `/version` 与 Core live proxy `/version` 通过 [真实环境]
  2026-06-02  M1-KUBEVIRT-LIVE-C KubeVirt VM real lab live result：VM lifecycle 通过，console/VNC subresource 要求 WebSocket upgrade；后续 D 已补齐真实 session [真实环境]
  2026-06-02  M1-NETWORK-LIVE-C Kube-OVN network real lab live result：resource gate 通过，join subnet 迁出 Tailscale CGNAT 段；后续 D 已补齐 external LB 可达性 [真实环境]
...
下一步：如需执行默认 StorageClass 切换、已有 PVC 迁移、HDD class 引入、备份/恢复演练、故障注入、长期 soak、升级/回滚演练、并发重启或更大故障演练，必须先由人工审批具体对象、预期影响和回滚方案。guard 已冻结不再新增。

历史回归门禁保留：`SDK-BETA-A`、`SDK-BETA-B`、`SDK-BETA-C`、`SDK-BETA-D`、`SDK-MOCK-SMOKE-A`、`SDK-MOCK-SMOKE-B`、`SDK-MOCK-SMOKE-C`、`SDK-MOCK-SMOKE-D`、`MOCK-A`、`DOC-API-A` 和 `SPRINT4-CLOSURE-A` 仍按 `repo/CURRENT-SPRINT.md` 验收命令执行。
```
...
| Sprint 5 ⭐ | ✅ 真实验证完成 | 计划窗口 2026-07-16~07-31 | 已完成功能批次见 `repo/CURRENT-SPRINT.md` 已完成切片；guard 微批次系列（299 个 validators）见 `repo/development-records/guard-series/REAL-K8S-LAB-guard-index.md`；三台物理服务器 K8s v1.36.1+Kube-OVN+KubeVirt bootstrap 完成；Kube-OVN network resource 与 external LoadBalancer IP 可达性、KubeVirt VM lifecycle 与 console/VNC WebSocket session、vCluster Helm/kubeconfig/Core proxy、vCluster upgrade、Secret、controller HA failover、KMS/SM4 provider streaming/objectstore round trip live gate、三节点 GPU 调度依赖与 CAPK node pool create/scale live gate 已真实执行并归档 evidence；验收命令与提交前门禁已通过 |
...
| Sprint 12 ⭐ | ✅ Core-only 已完成 | 2026-06-19 | Core「Services 支撑 Handler」实现已收口；A `SPRINT12-KICKOFF-A`、B1 `CORE-SVC-SUPPORT-OBSERVABILITY-A`、B2 `CORE-SVC-SUPPORT-NETSTORE-A`、B3 `CORE-SVC-SUPPORT-OBJVEC-A` 与 `SPRINT12-CLOSURE-A` 已完成；19 个 Core handler + 2 个 422 均关联 `api/openapi/v1.yaml` operationId、`pkg/ports`、`pkg/adapters` 与 Gateway handler；仅 Tier1 local profile，不代表 runtime/production ready |
| Sprint 13 ⭐ | 进行中 | 2026-06-19 起 | 真实 provider / live gate 收敛；从 Sprint 12 已建立的 ports/adapters/router 边界接入 K8s/kubelet/Prometheus/DCGM/Kata/Kube-OVN/Rook-Ceph/MinIO/向量后端等真实组件；S01 网络路由 Kube-OVN、S02 K8s workloads vCluster、S03 storage Rook-Ceph 与 S04 GPU NVIDIA device-plugin/DCGM 已通过 production-shaped live gate 并归档 `production_shape.status=passed` evidence；S05 object-store MinIO、S06 vector Milvus 与 S07 instance observability Prometheus 已到 code+contract ready / LIVE PENDING（`SPRINT13-INSTANCE-OBSERVABILITY-PROMETHEUS-A-TRACK` / `validate-instance-observability-live-gate` 固定 Prometheus + kubelet / K8s API logs/events/metrics/security-events/exec contract）；计划见 `repo/development-records/sprint13-real-provider-readiness-plan.md`，未执行对应 production-shaped live gate 前不得标 runtime/production ready |
...
| Sprint 5 ⭐ | ✅ 真实验证完成 | K8s 集群/proxy/upgrade/node-pool、加解密(KMS/SM4)、Secrets、reconcile(含 leader election) 的**契约 + local profile + 代码边界 + live gate 契约门禁全部完成**；Kube-OVN network resource 与 external LoadBalancer IP 可达性、KubeVirt VM lifecycle 与 console/VNC WebSocket session live gate、vCluster Helm/kubeconfig/Core proxy live gate、vCluster upgrade live gate、Secret live gate（含 VM guest Secret volume 可见性）、controller HA failover live gate、KMS/SM4 provider streaming/objectstore round trip live gate、三节点 GPU 调度依赖与 CAPK node pool create/scale live gate 已在真实 lab 跑通并归档 evidence；验收命令和提交前门禁已通过 |

物理底座：三台开发服务器已完成 K8s `v1.36.1` + Kube-OVN `v1.15.8` + KubeVirt `v1.8.2` 最小部署；Kube-OVN CNI join subnet 已迁移到 `172.30.0.0/16` 并三节点 Ready；Kube-OVN network resource 与 external LoadBalancer IP 可达性、KubeVirt VM lifecycle 与 console/VNC WebSocket session live gate、vCluster Helm/kubeconfig/Core proxy live gate、vCluster upgrade live gate、Secret live gate（含 VM guest Secret volume 可见性）、controller HA failover live gate、KMS/SM4 provider streaming/objectstore round trip live gate、三节点 GPU 调度依赖与 CAPK node pool provider-backed create/scale live gate 已跑通。

**→ 继续入口：按 [`repo/CURRENT-SPRINT.md`](repo/CURRENT-SPRINT.md) 查看 Sprint 11 当前切片、验收命令、冻结目录和真实部署结果；后续破坏性磁盘操作、默认 StorageClass 切换、已有 PVC 迁移、HDD class 引入、并发重启或更大故障演练仍必须单独审批。**

### 当前真实底座环境状态

三台物理开发服务器已完成 Kubernetes `v1.36.1` 集群、Kube-OVN `v1.15.8` 和 KubeVirt `v1.8.2` 最小组件部署；Kube-OVN CNI/CoreDNS Ready，KubeVirt phase `Deployed`。Kube-OVN network resource live gate 与 external LoadBalancer IP 可达性已通过并归档 evidence；KubeVirt VM lifecycle 与 console/VNC WebSocket session live gate 已通过并归档 evidence，console/VNC 均完成 HTTP `101` upgrade、`plain.kubevirt.io` 子协议回选和流数据字节验证；vCluster Helm/kubeconfig/Core proxy live gate、vCluster upgrade live gate、Secret live gate（含 VM guest Secret volume 可见性）、controller HA failover live gate、KMS/SM4 provider streaming/objectstore round trip live gate、三节点 GPU 调度依赖与 CAPK node pool provider-backed create/scale live gate 已通过并归档 evidence，Core proxy 本次经本机 kubectl proxy 转发到 live vCluster，不代表生产 per-cluster metadata target/KMS token 管理已完成；Controller HA 本次使用最小 live gate 依赖和 hostPath worker 二进制，不代表生产 Helm/Operator 化控制面部署已完成；KMS/SM4 本次使用 live-gate fixture，不代表生产 KMS/对象存储、直连 TLS/credential 管理或平台化部署完成；Kube-OVN external LB 本次使用 live-gate helper 镜像/脚本兼容方案，不代表生产镜像供应链或 Helm/Operator 化部署完成；CAPK node pool 本次证明 VM worker create/scale-ready，不代表 VM 内 GPU passthrough/vGPU 已完成。详细记录见 [`repo/development-records/real-k8s-lab-k8s-kubeovn-kubevirt-bootstrap.md`](repo/development-records/real-k8s-lab-k8s-kubeovn-kubevirt-bootstrap.md)、[`repo/development-records/m1-network-live-c-kubeovn-real-lab-result.md`](repo/development-records/m1-network-live-c-kubeovn-real-lab-result.md)、[`repo/development-records/m1-network-live-d-kubeovn-external-lb-real-lab-result.md`](repo/development-records/m1-network-live-d-kubeovn-external-lb-real-lab-result.md)、[`repo/development-records/m1-kubevirt-live-c-vm-real-lab-result.md`](repo/development-records/m1-kubevirt-live-c-vm-real-lab-result.md)、[`repo/development-records/m1-kubevirt-live-d-console-vnc-session-real-lab-result.md`](repo/development-records/m1-kubevirt-live-d-console-vnc-session-real-lab-result.md)、[`repo/development-records/m1-k8s-live-g-vcluster-real-lab-result.md`](repo/development-records/m1-k8s-live-g-vcluster-real-lab-result.md)、[`repo/development-records/m1-k8s-live-h-vcluster-upgrade-real-lab-result.md`](repo/development-records/m1-k8s-live-h-vcluster-upgrade-real-lab-result.md)、[`repo/development-records/m1-k8s-live-k-gpu-scheduling-real-lab-progress.md`](repo/development-records/m1-k8s-live-k-gpu-scheduling-real-lab-progress.md)、[`repo/development-records/m1-k8s-live-m-node-pool-capk-real-lab-result.md`](repo/development-records/m1-k8s-live-m-node-pool-capk-real-lab-result.md)、[`repo/development-records/m1-secrets-live-c-secret-real-lab-result.md`](repo/development-records/m1-secrets-live-c-secret-real-lab-result.md)、[`repo/development-records/m1-secrets-live-d-vm-secret-guest-real-lab-result.md`](repo/development-records/m1-secrets-live-d-vm-secret-guest-real-lab-result.md)、[`repo/development-records/m1-reconcile-live-c-ha-real-lab-result.md`](repo/development-records/m1-reconcile-live-c-ha-real-lab-result.md) 与 [`repo/development-records/m1-encrypt-live-c-kms-sm4-real-lab-result.md`](repo/development-records/m1-encrypt-live-c-kms-sm4-real-lab-result.md)。
...
```
           ANI Core 开发线                         外部 Services/产品线
           ──────────────────────────────────      ──────────────────────────
S1  05-15~05-31  操作语义底座 + Health + Auth收尾  ✅ 已完成
...
S8  09-01~09-15  Core 收尾/发布前加固              ⏳ 计划中；Console/BOSS/RAG 由外部团队负责
S9  09-16~09-25  RC 加固（只修 Bug）               ⏳ 计划中
S10 09-26~09-30  v1.0.0 发布                       ⏳ 计划中
──────────────────────────────────────────────────────────────────────────────
09-30  目标：ANI Core v1.0.0；Services P0 由外部团队交付，不在本仓库开发
```

### 代码依赖关键路径（实际代码状态驱动）

> 以下基于 2026-06-04 代码与文档状态，反映真实依赖而非愿景描述；当前仓库只推进 ANI Core。
...
当前代码实际状态：
  ...
  ⚠️  model-service 属于 ANI Services 早期逻辑，只读冻结，只能作为历史参考
  ⚠️  kb-service 当前为空目录，不能被文档描述成已实现知识库服务
  ⚠️  frontends/console 当前仍有历史单 API Client 使用痕迹；已冻结并移交外部产品/Services 团队
...
关键依赖链（必须按顺序）：
  ...
  Sprint 7：Core installer、离线包 manifest、Core CLI minimal behavior、Core regression profile 已完成
       ↓
  Sprint 8~10：Core 收尾/发布前加固、RC 和 v1.0.0 发布；RAG/Console/BOSS/Services 业务由外部团队负责
```
...
> 2026-06-04 校准：Sprint 7 Core-only 代码开发已完成。旧路线图中的知识库 RAG、Console Alpha、model-service、kb-service、ai、operators 和 frontends 均不在本仓库执行范围；外部 Services 团队负责业务能力与前端。本仓库只按 Core OpenAPI/SDK 缺口补齐基础设施支撑能力。
...
不在本仓库执行。知识库 RAG、Console Alpha、BOSS、model-service、kb-service、ai、operators 和 frontends 均由外部 Services/产品团队负责；本仓库不得基于猜测提前开发这些目录或把 Services 业务资源回流到 Core API。
