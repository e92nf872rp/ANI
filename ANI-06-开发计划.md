...
> 最后更新：2026-05-25（补充 REAL-K8S component report overall status）
...
```
当前阶段：Phase 1 / Sprint 5 收敛中（K8s、加解密、Secret local profile 主链路已完成，vCluster Helm provider 代码边界、vCluster kubeconfig provider 代码边界、K8s cluster upgrade API/provider 代码边界、K8s node pool CRUD local profile、Cluster API node pool provider 代码边界、M1-K8S-LIVE-A vCluster live 验证门禁、M1-K8S-LIVE-D vCluster live evidence JSON 输出、M1-K8S-LIVE-B node pool live 验证门禁、M1-K8S-LIVE-F node pool evidence JSON 输出、M1-K8S-LIVE-C vCluster upgrade live 验证门禁、M1-K8S-LIVE-E vCluster upgrade evidence JSON 输出、M1-NETWORK-LIVE-A Kube-OVN Vpc/Subnet live 验证门禁、M1-NETWORK-LIVE-B Kube-OVN network evidence JSON 输出、M1-KUBEVIRT-LIVE-A KubeVirt VM live 验证门禁、M1-KUBEVIRT-LIVE-B KubeVirt VM evidence JSON 输出、K8s proxy forwarding adapter、target resolver/store、metadata 持久化、Gateway router 注入接线与 forwarding_static/forwarding_metadata runtime 选择已完成，KMS/SM4 HTTP provider 代码边界、对象内容 SM4-GCM 流式加解密代码边界、M1-ENCRYPT-LIVE-A KMS/SM4 provider streaming live 验证门禁和 M1-ENCRYPT-LIVE-B KMS/SM4 evidence JSON 输出已完成，Kubernetes Secret provider 写入代码边界、容器/Job Secret binding env/file manifest 注入代码边界、VM Secret binding volume manifest 注入代码边界、M1-SECRETS-LIVE-A Kubernetes Secret env/file/VM live 验证门禁和 M1-SECRETS-LIVE-B Kubernetes Secret evidence JSON 输出已完成，controller 已具备默认关闭的 bootstrap opt-in 运行剖面、目标级失败退避、计数快照、`/metrics` Prometheus text 指标导出、独立 worker 进程形态、metadata-backed leader election 代码边界、M1-RECONCILE-LIVE-A controller HA live 验证门禁和 M1-RECONCILE-LIVE-B controller HA evidence JSON 输出，REAL-K8S-LAB-A 已索引组件级 contract gates、支持 `--live` evidence JSON 输出，并具备 M1-REAL-LAB-D component live runner、M1-REAL-LAB-E component live 聚合失败摘要、M1-REAL-LAB-F required env preflight、M1-REAL-LAB-G component env template、M1-REAL-LAB-H component env file loader、M1-REAL-LAB-I component preflight-only mode、M1-REAL-LAB-J component gate selector、M1-REAL-LAB-K component summary report、M1-REAL-LAB-L component report stale summary guard、M1-REAL-LAB-M component report diagnostic details、M1-REAL-LAB-N component evidence integrity guard、M1-REAL-LAB-O component evidence content guard、M1-REAL-LAB-P component report passed-evidence audit、M1-REAL-LAB-Q component report unresolved exit guard 和 M1-REAL-LAB-R component report overall status，真实 provider live 验证未完成）
...
刚完成：Sprint 5 Core dev/local profile 主链路切片、vCluster Helm provider 代码边界、vCluster kubeconfig provider 代码边界、K8s cluster upgrade API/provider 代码边界、K8s node pool CRUD local profile、Cluster API node pool provider 代码边界、M1-K8S-LIVE-A vCluster live 验证门禁（validate-vcluster-live-gate）、M1-K8S-LIVE-D vCluster live evidence JSON 输出、M1-K8S-LIVE-B node pool live 验证门禁（validate-k8s-node-pool-live-gate）、M1-K8S-LIVE-F node pool evidence JSON 输出、M1-K8S-LIVE-C vCluster upgrade live 验证门禁（validate-vcluster-upgrade-live-gate，覆盖 `controlPlane.distro.k8s.version` 检查步骤）、M1-K8S-LIVE-E vCluster upgrade evidence JSON 输出、M1-NETWORK-LIVE-A Kube-OVN `Vpc/Subnet` live 验证门禁（validate-kubeovn-network-live-gate）、M1-NETWORK-LIVE-B Kube-OVN network evidence JSON 输出、M1-KUBEVIRT-LIVE-A KubeVirt VM live 验证门禁（validate-kubevirt-vm-live-gate，覆盖 stop 与 console/VNC 检查步骤）、M1-KUBEVIRT-LIVE-B KubeVirt VM evidence JSON 输出、K8s proxy forwarding adapter/target resolver/metadata 持久化/Gateway router 注入接线/forwarding_static 与 forwarding_metadata runtime 选择、KMS/SM4 HTTP provider 代码边界、对象内容 SM4-GCM 流式加解密代码边界、M1-ENCRYPT-LIVE-A KMS/SM4 provider streaming live 验证门禁（validate-kms-sm4-live-gate）、M1-ENCRYPT-LIVE-B KMS/SM4 evidence JSON 输出、Kubernetes Secret provider 写入代码边界、Workload Secret binding env/file/VM volume manifest 注入代码边界、M1-SECRETS-LIVE-A Kubernetes Secret env/file/VM live 验证门禁（validate-secrets-live-gate）、M1-SECRETS-LIVE-B Kubernetes Secret evidence JSON 输出、controller 退避/计数快照/Prometheus text 指标/独立 worker/metadata-backed leader election、M1-RECONCILE-LIVE-A controller HA live 验证门禁（validate-reconcile-ha-live-gate，覆盖 `control_plane_leases` holder 切换与 HA failover 检查步骤）、M1-RECONCILE-LIVE-B controller HA evidence JSON 输出、真实底座验证门禁、M1-REAL-LAB-B 组件级 contract gate 索引、M1-REAL-LAB-C `--live` evidence JSON 输出、M1-REAL-LAB-D component live runner、M1-REAL-LAB-E component live 聚合失败摘要、M1-REAL-LAB-F required env preflight、M1-REAL-LAB-G component env template、M1-REAL-LAB-H component env file loader、M1-REAL-LAB-I component preflight-only mode、M1-REAL-LAB-J component gate selector、M1-REAL-LAB-K component summary report、M1-REAL-LAB-L component report stale summary guard、M1-REAL-LAB-M component report diagnostic details、M1-REAL-LAB-N component evidence integrity guard、M1-REAL-LAB-O component evidence content guard、M1-REAL-LAB-P component report passed-evidence audit、M1-REAL-LAB-Q component report unresolved exit guard 和 M1-REAL-LAB-R component report overall status（2026-05-25，k8s-clusters CRUD+kubeconfig+proxy+node-pools local profile + vCluster Helm/kubeconfig/upgrade provider boundary + Cluster API node pool provider boundary + vCluster live gate + vCluster evidence output + node pool live gate + node pool evidence output + vCluster upgrade live gate + vCluster upgrade evidence output + Kube-OVN Vpc/Subnet live gate + Kube-OVN network evidence output + KubeVirt VM live gate + KubeVirt VM evidence output + proxy forwarding adapter/target resolver/metadata persistence/Gateway injection/forwarding_static/forwarding_metadata + encryption keys/seal/unseal-token/rotate/revoke + KMS/SM4 HTTP provider boundary + SM4-GCM object content streaming boundary + KMS/SM4 provider streaming live gate + KMS/SM4 evidence output + secrets CRUD/bindings + Kubernetes Secret provider write boundary + container/job/vm Secret binding manifest injection + Secret live gate + Secret evidence output + WorkloadReconcileController opt-in 运行剖面/目标级失败退避/计数快照/metrics/worker/leader election + reconcile HA live gate + reconcile HA evidence output + REAL-K8S-LAB-A + component live runner + component live aggregation + required env preflight + env template + env file loader + preflight-only mode + component gate selector + component summary report + stale summary guard + report diagnostic details + evidence integrity guard + evidence content guard + report passed-evidence audit + report unresolved exit guard + report overall status）
...
| Sprint 5 ⭐（当前） | 🔄 部分完成 | 2026-05-25 完成 K8s/Encryption/Secrets local profile 主链路切片、vCluster Helm provider 代码边界、vCluster kubeconfig provider 代码边界、K8s cluster upgrade API/provider 代码边界、K8s node pool CRUD local profile、Cluster API node pool provider 代码边界、M1-K8S-LIVE-A vCluster live 验证门禁、M1-K8S-LIVE-D vCluster live evidence JSON 输出、M1-K8S-LIVE-B node pool live 验证门禁、M1-K8S-LIVE-F node pool evidence JSON 输出、M1-K8S-LIVE-C vCluster upgrade live 验证门禁、M1-K8S-LIVE-E vCluster upgrade evidence JSON 输出、M1-NETWORK-LIVE-A Kube-OVN Vpc/Subnet live 验证门禁、M1-NETWORK-LIVE-B Kube-OVN network evidence JSON 输出、M1-KUBEVIRT-LIVE-A KubeVirt VM live 验证门禁、M1-KUBEVIRT-LIVE-B KubeVirt VM evidence JSON 输出、K8s proxy forwarding adapter/target resolver/metadata 持久化/Gateway router 注入接线/forwarding_static 与 forwarding_metadata runtime 选择、KMS/SM4 HTTP provider 代码边界、对象内容 SM4-GCM 流式加解密代码边界、M1-ENCRYPT-LIVE-A KMS/SM4 provider streaming live 验证门禁、M1-ENCRYPT-LIVE-B KMS/SM4 evidence JSON 输出、Kubernetes Secret provider 写入代码边界、Workload Secret binding env/file/VM volume manifest 注入代码边界、M1-SECRETS-LIVE-A Kubernetes Secret env/file/VM live 验证门禁、M1-SECRETS-LIVE-B Kubernetes Secret evidence JSON 输出、WorkloadReconcileController opt-in 运行剖面、目标级失败退避/计数快照/Prometheus text 指标/独立 worker、metadata-backed leader election、M1-RECONCILE-LIVE-A controller HA live 验证门禁、M1-RECONCILE-LIVE-B controller HA evidence JSON 输出、REAL-K8S-LAB-A contract gate、`--live` evidence JSON 输出、M1-REAL-LAB-D component live runner、M1-REAL-LAB-E component live 聚合失败摘要、M1-REAL-LAB-F required env preflight、M1-REAL-LAB-G component env template、M1-REAL-LAB-H component env file loader、M1-REAL-LAB-I component preflight-only mode、M1-REAL-LAB-J component gate selector、M1-REAL-LAB-K component summary report、M1-REAL-LAB-L component report stale summary guard、M1-REAL-LAB-M component report diagnostic details、M1-REAL-LAB-N component evidence integrity guard、M1-REAL-LAB-O component evidence content guard、M1-REAL-LAB-P component report passed-evidence audit、M1-REAL-LAB-Q component report unresolved exit guard 和 M1-REAL-LAB-R component report overall status；计划窗口 2026-07-16~07-31 | 已完成 k8s-clusters CRUD+kubeconfig+proxy+node-pools local profile、vCluster Helm/kubeconfig/upgrade provider 代码边界、Cluster API node pool provider 代码边界、validate-vcluster-live-gate contract 门禁、vCluster live evidence JSON 输出、validate-k8s-node-pool-live-gate contract 门禁、node pool evidence JSON 输出、validate-vcluster-upgrade-live-gate contract 门禁、vCluster upgrade evidence JSON 输出、validate-kubeovn-network-live-gate contract 门禁、Kube-OVN network evidence JSON 输出、validate-kubevirt-vm-live-gate contract 门禁、KubeVirt VM evidence JSON 输出、proxy forwarding adapter/target resolver/metadata 持久化/Gateway router 注入接线/forwarding_static 与 forwarding_metadata runtime 选择、encryption keys/seal/unseal-token/rotate/revoke、KMS/SM4 HTTP provider 代码边界、对象内容 SM4-GCM 流式加解密代码边界、validate-kms-sm4-live-gate contract 门禁、KMS/SM4 evidence JSON 输出、secrets CRUD/bindings local profile、Kubernetes Secret provider 写入代码边界、容器/Job Secret binding env/file manifest 注入代码边界、VM Secret binding volume manifest 注入代码边界、validate-secrets-live-gate contract 门禁、Kubernetes Secret evidence JSON 输出，以及 controller 默认关闭的 bootstrap 后台运行、目标级失败退避、`/metrics` 指标导出、独立 worker、metadata-backed leader election 代码边界、validate-reconcile-ha-live-gate contract 门禁、controller HA evidence JSON 输出、REAL-K8S-LAB-A live evidence JSON 输出、component live runner、component live 聚合失败摘要、required env preflight、component env template、env file loader、preflight-only mode、component gate selector、component summary report、stale summary guard、report diagnostic details、component evidence integrity guard、component evidence content guard、component report passed-evidence audit、component report unresolved exit guard 和 component report overall status；vCluster live Helm 安装验证、真实 kubeconfig 可用性、live proxy 验证、live vCluster 升级验证真实执行结果、真实 Kube-OVN Vpc/Subnet 与 NetworkPolicy/Service LB 验证结果、KubeVirt VM lifecycle 与 console/VNC live 验证结果、真实节点池 live 扩缩容、GPU 节点池真实调度验证、controller 多副本 live HA failover 验证真实执行结果、KMS/SM4 live backend 验证、对象存储 + KMS/SM4 provider streaming 端到端验收、Kubernetes Secret live 写入验证和实例 Secret env/file/VM volume live 注入验证真实执行结果尚未完成 |
...
| 批次 | 优先级 | 状态 |
|---|---|---|
| M1-K8S-A/B/C/D/E/F/G + M1-K8S-LIVE-A/B/C/D/E/F + M1-K8S-PROXY-A/B/C/D/E/F | P0 ⭐ | ✅ 已完成：`/api/v1/k8s-clusters` create/get/list/delete + kubeconfig + proxy + node-pools local profile、vCluster Helm provider 代码边界、vCluster kubeconfig provider 代码边界、`POST /api/v1/k8s-clusters/{cluster_id}/upgrade` upgrade API/provider 代码边界、Cluster API node pool provider 代码边界、Gateway `K8S_CLUSTER_NODE_POOL_PROVIDER_MODE=clusterapi_kubernetes_rest` runtime 选择、`validate-vcluster-live-gate` contract 门禁、vCluster live evidence JSON 输出、`validate-k8s-node-pool-live-gate` contract 门禁、node pool evidence JSON 输出、`validate-vcluster-upgrade-live-gate` contract 门禁、vCluster upgrade evidence JSON 输出、可注入 resolver 的 proxy forwarding adapter、per-cluster target resolver/store、proxy target metadata 持久化、Gateway router 注入接线、Gateway `forwarding_static` / `forwarding_metadata` runtime 选择；未完成 live Helm 安装验证、真实 kubeconfig 可用性、live proxy 验证、live vCluster 升级验证真实执行结果、真实节点池 live 扩缩容和 GPU 节点池真实调度验证 |
| M1-NETWORK-LIVE-A/B | P0 ⭐ | ✅ 已完成 contract gate 与 evidence JSON 输出：`validate-kubeovn-network-live-gate` 固定 Kube-OVN `Vpc/Subnet`、NetworkPolicy 和 Service/LB 检查入口，并支持 `--evidence-output` / `ANI_KUBEOVN_NETWORK_LIVE_EVIDENCE_OUTPUT` 归档 JSON 证据；未完成真实 lab live 执行结果，不得标记为网络真实 provider 完成 |
| M1-KUBEVIRT-LIVE-A/B | P0 ⭐ | ✅ 已完成 contract gate 与 evidence JSON 输出：`validate-kubevirt-vm-live-gate` 固定 KubeVirt VM start/stop lifecycle、console/VNC 和 delete 检查入口，并支持 `--evidence-output` / `ANI_KUBEVIRT_VM_LIVE_EVIDENCE_OUTPUT` 归档 JSON 证据；未完成真实 lab live 执行结果，不得标记为 VM 真实 provider 完成 |
| M1-ENCRYPT-A/B/C/D + M1-ENCRYPT-LIVE-A/B | P0 | ✅ 已完成：`/api/v1/encryption/keys` create/get/list/delete + seal + unseal-token + rotate + revoke、OpenAPI schema、ports、local runtime service、租户隔离、幂等 create/seal/rotate/revoke、SDK 生成和 router 单元测试；已完成 `ports.EncryptionProvider`、KMS/SM4 HTTP provider adapter、provider-backed evidence、Gateway `ENCRYPTION_PROVIDER_MODE=kms_sm4_http` runtime 选择、对象内容 SM4-GCM 流式加解密代码边界、`validate-kms-sm4-live-gate` contract 门禁和 `--live --evidence-output` JSON 证据输出；未完成 live KMS/SM4 backend 验证和对象存储 + provider streaming 端到端验收 |
| M1-SECRETS-A/B/C/D + M1-SECRETS-LIVE-A/B | P0 | ✅ 已完成：`/api/v1/secrets` create/get/list/delete + bindings local profile、OpenAPI schema、ports、local runtime service、租户隔离、幂等创建、SDK 生成和 router 单元测试；已完成 Kubernetes Secret provider 写入代码边界、Gateway `SECRET_PROVIDER_MODE=kubernetes_rest` runtime 选择、容器/Job Secret binding env/file manifest 注入代码边界、VM Secret binding volume manifest 注入代码边界、`validate-secrets-live-gate` contract 门禁和 Kubernetes Secret evidence JSON 输出；响应不返回明文；未完成 live 写入验证和实例 Secret env/file/VM volume live 注入验证真实执行结果 |
| M1-K8S-A 后续切片 | P0 ⭐ | ⏳ 未完成：执行 M1-K8S-LIVE-A live 模式证明 vCluster live Helm 安装、真实 kubeconfig 可用性和 live proxy；执行 M1-K8S-LIVE-C live 模式证明 live vCluster upgrade；执行 M1-K8S-LIVE-B live 模式并用 `--evidence-output` 证明真实节点池 live 扩缩容和 GPU 节点池真实调度验证 |
| M1-RECONCILE-A/B/C/D/E + M1-RECONCILE-LIVE-A/B | P0 | ✅ 已完成基础闭环：controller adapter/capability + `bootstrap.RunGRPC` opt-in 后台运行 + 目标级失败退避、计数快照、`/metrics` Prometheus text 指标导出、独立 worker 进程形态、metadata-backed leader election 代码边界、`validate-reconcile-ha-live-gate` contract 门禁和 `--live --evidence-output` JSON 证据输出；未完成多副本 live HA failover 验证真实执行结果 |
| REAL-K8S-LAB-A / M1-REAL-LAB-B/C/D/E/F/G/H/I/J/K/L/M/N/O/P/Q/R | P0 ⭐ | ✅ 已完成 contract gate：`deploy/real-k8s-lab/profile.yaml`、`make validate-real-k8s-profile`、live kubectl 检查入口、组件级 `contract_gates` 索引、`--live --evidence-output` JSON 证据输出，以及 `--component-live` 组件 gate 统一 runner、失败聚合摘要、required env preflight、component env template、component env file loader、`--component-preflight` preflight-only mode、`--component-gate` 单 gate selector、`--component-report` summary 报告、stale summary guard、report diagnostic details、component evidence integrity guard、component evidence content guard、component report passed-evidence audit、component report unresolved exit guard 和 component report overall status；真实三台云 VM 环境尚未部署 |
| M1-ENCRYPT-A 后续切片 | P0 | ⏳ 未完成：执行 `validate-kms-sm4-live-gate --live --evidence-output` 证明 KMS/SM4 live backend、真实 provider 下的端到端密钥生命周期验收、对象存储 + KMS/SM4 provider streaming 端到端验收 |
| M1-SECRETS-A 后续切片 | P0 | ⏳ 未完成：执行 `validate-secrets-live-gate --live --evidence-output` 证明 Kubernetes Secret live 写入、实例 Secret env/file/VM volume live 注入验证 |

**→ 继续入口：按 [`repo/CURRENT-SPRINT.md`](repo/CURRENT-SPRINT.md) 继续 Sprint 5；下一个最小可验收闭环优先用 REAL-K8S-LAB-A `--component-env-template-output` 生成并填充 `contract_gates[].required_env` 对应环境，再通过 `--component-preflight --component-env-file ... --evidence-output ...` 检查配置完整性，可用 `--component-gate <id>` 只检查或重跑单个失败 gate，最后执行 `--component-live --component-env-file ...` 汇总组件 live gates（summary 会记录 total/passed/failed/blocked 并在失败时非零退出），若 summary 有 failed/blocked gate，或 passed gate 声明的 `evidence_output` 缺失、malformed、non-passing，则用 `--component-report <summary.json>` 生成 selected preflight/live 复跑命令；report 写出后包含 passed 与 unresolved_gates，仍有 failed/blocked gate 时 CLI 非零退出；或单独执行 M1-NETWORK-LIVE-A/B Kube-OVN live 验证并归档 evidence JSON、M1-KUBEVIRT-LIVE-A/B KubeVirt VM live 验证并归档 evidence JSON、M1-K8S-LIVE-A live Helm/kubeconfig/live proxy 验证、M1-K8S-LIVE-C live vCluster upgrade 验证、M1-K8S-LIVE-B live 节点池扩缩容/GPU 调度验证并归档 evidence JSON、M1-RECONCILE-LIVE-A/B controller HA failover live 验证并归档 evidence JSON，或 Encryption/Secrets 真实 provider。**
