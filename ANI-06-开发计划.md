...
- INFERENCE-SERVICE-ENGINE-EXTRA-ARGS-CONTRACT-C35：已补齐 Services 创建契约可选 `engine.extra_args`；创建时冻结、响应只读回显；保留名由后续实现返回 400 INVALID_ARGUMENT；不进入 PATCH。不含 handler/proto/`engine.Launch`。无新 live，不得外推 GPU ready / runtime ready。
