...
> ANI IAM Direct P2：DP2-02 已在固定来源 `0cedae825a489d936cf41815dc27f278f6d3213c` 的专用本地分支完成并经人工接受公网 OpenAPI/operation registry breaking freeze。目标为 295 operations（59 新增、0 删除、6 operationId 变化）与 45 个新增 schema；Console/BOSS 类型及四语言 Core SDK 同源生成。Gateway 运行时接线、实际新增 Handler、调用方切换与部署仍为 `not_verified`，该本地批次不切流、不发布远端 Artifact。
> ANI IAM Direct P2：DP2-03 已经人工接受精确 Proto/Core contract diff，冻结目标 `iam.v1` Authentication/Authorization/Admin 3 services / 69 RPC，以及 Core-owned `tenant.integration.v1` Lifecycle/Heartbeat/Bootstrap/Snapshot contract、只读 2-RPC snapshot service、descriptors、pins 和八组 producer-consumer fixtures。契约、生成与相关回归为 `pass`；ANI 聚合旧 DP2-02 operationId gate 为 `fail`；运行时 registration、Core/NATS、Gateway mapping/cutover 和部署均为 `not_verified`。本地批次不发布、不部署、不切流。
> ANI IAM Direct P2：DP2-05 已在专用本地 worktree 形成目标最小纵向链路工程证据。真实 PostgreSQL/Redis、受限 runtime role、真实 IAM/Gateway 进程、Console/Tenant Password/Session/Grant/Access Token、`listInstances` 一次 CheckPermission、401/403/503/504、Audit 原子、two-Tenant/query mutation、空库重放和人工接受的 Go/No-Go A Go 结论均为 `pass`；BOSS/Platform Password Login 与 BOSS caller E2E 为 `not_verified`。本地批次未 push、未提交到 ANI 主线、未部署、未切流、未删除旧 Auth、未启动 DP2-06；统一 24 小时 Idempotency Ledger 仍按已接受 graph 归 DP2-16。
> ANI IAM Direct P2 安全合并：固定 a221a7b/573d373/f09a436 已在专用 integration worktree 以 `--no-ff --no-commit` 整合到人工接受的 main `bde4ea72b5a91cd43cc271dd44c09ff262c637e5`。默认/disabled 继续旧 Auth，`dp2_05` 仅隔离 Password Login/`listInstances` tracer；Target IAM 已迁移为 `pkg/ports` + `pkg/adapters/iam` 并使用公共固定 protobuf 生成包。安全修复、生成幂等与全部本地强制门禁为 `pass`；Standards/Spec 双轴评审均为 0 findings，Merge-Ready 已于 2026-09-06 人工接受，80-path staged package 与检查点后复验均通过。本记录随本地 merge commit 落地；未 push、未部署、未切流、未启动 DP2-06。
...
租户计费结算接口：TENANT-BILLING-A（2026-09-08，local verified，Services PR）——BOSS 租户计费结算 5 端点 `/api/v1/svc/billing/*`（总览/CSV 导出/生成账单/账单动作 settle·credit/调账）按方案 v0.3 落地：Services 层归属（契约 `services/v1.yaml` + x-ani-authz boundary=platform，实现 tenant-service gRPC，网关 mixed handler 转发）；生成即出账（无 draft）、一期一单幂等、overdue 读取时动态计算不落库、余额读取推导、单价只读 `billing_pricing`（种子 ON CONFLICT DO NOTHING，代码零价格字面量）、用量仅经 Core SDK 调 `/metering/usage/platform` 不直查 Core 库、仅 token_total 计费。本地集成实测 21 用例通过（测试环境占用未部署 K8s，不标 live/runtime ready）；真环境 RBAC 回归与成本对账留待后续批次。批次详情 `repo/development-records/TENANT-BILLING-A.md`。当前执行入口：repo/CURRENT-SPRINT.md
