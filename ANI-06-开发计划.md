...
- 实例交互式终端 WebSocket：INSTANCE-EXEC-WS-A + INSTANCE-EXEC-KUBERNETES-STREAM-A；`POST /api/v1/instances/{id}/exec` 返回短期 token 与内嵌 token 的浏览器直连 `ws_url`，新增 `GET /api/v1/instances/{id}/exec/{session}` WebSocket 握手入口；RBAC 固定 `scope:instances:exec`，WebSocket 使用 KubeCloud TerminalMessage JSON 帧，终端可见输出统一 `Op=stdout` 并兼容旧裸 stdin 输入；Prometheus/Kubernetes observability adapter 已补真实容器 exec stream：Gateway 委托 `InstanceExecSessionConnector`，Kubernetes `/pods/{pod}/exec` 使用 `v4.channel.k8s.io`，stdin/resize 映射 channel 0/4，stdout/stderr channel 1/2 映射回 `Op=stdout`；local/logic verified，不放宽鉴权；集群暂不可连，live evidence 待补。
- Sandbox Kata workload runtime：SANDBOX-KATA-WORKLOAD-RUNTIME-A；Gateway `kind=sandbox` 在 `WORKLOAD_PROVIDER=kubernetes_rest` 下复用既有 WorkloadRuntime / Kubernetes provider orchestration，使用 `runtimeClassName=sandbox-kata` 创建 Deployment；真实 live smoke 已通过（Pod Running/Ready，guest kernel 日志可见），脱敏 evidence 已归档；API DELETE 语义和 release/operator gate 未覆盖，不声明 full platform production ready。
- VM Console / noVNC WebSocket：INSTANCE-CONSOLE-VNC-WS-A；修复创建 session 时对 KubeVirt `/vnc` 的普通 HTTP GET 探测；`POST /api/v1/instances/{id}/console` 返回内嵌 token 的 `connect_url`，新增 `GET /api/v1/instances/{id}/console/{session}`；Gateway 以 `plain.kubevirt.io` 转发到 KubeVirt，供浏览器 noVNC 直连；VM-NOVNC-USB-TABLET-A 默认注入 KubeVirt USB tablet absolute pointer，减少 noVNC 鼠标坐标偏移；local/logic verified，不改 Services/前端，不标 production ready。
- ISO/CDI image upload foundation：CDI-FOUNDATION-A；isolated foundation 增加 CDI `v1.65.0`，顺序为 KubeVirt → CDI → Rook → Harbor；operator manifest 中 CDI 组件镜像指向 `docker.changqingyun.cn/mirror/kubevirt/*:v1.65.0`，CDI CR 使用 `ani-rbd-ssd` scratch StorageClass，`cdi-uploadproxy-nodeport` 暴露 NodePort `31001`；仅为后续 ISO 直传/DataVolume/VM CD-ROM boot 批次铺底，不实现 Images API 或 VM ISO boot，不标 production ready。
- ISO/CDI image upload ports：IMAGE-UPLOAD-CDI-A；新增 `ImageImportService` port 与 local adapter，对齐 OpenAPI `Image` / `CreateImageUploadRequest` / `ImageUploadSession` schema，local profile 支持 `iso` 上传会话、幂等、Get/List/Delete，非 `iso` 返回 `ErrUnsupported`；不实现 CDI adapter、Gateway routes 或 VM ISO boot，不标 runtime/production ready。
...
- ISO VM CD-ROM detach：ISO-CDI-VM-CDROM-DETACH-A；`detach_volume volume_id=iso` 在 VM `cdrom` 场景调用 Kubernetes lifecycle provider merge-patch 删除 `iso`；后续引导语义由 ISO-VM-BOOTORDER-DISK-FIRST-A 收口；local/logic verified，live 待验证，不标 production ready。
- ISO VM bootOrder disk-first：ISO-VM-BOOTORDER-DISK-FIRST-A；修正仅 rootdisk 有 bootOrder 导致刚创建进不了 ISO 的问题；渲染为 `rootdisk.bootOrder=1` + `iso.bootOrder=2`，装完访客自重启进系统无需删 pod/detach；`boot_media.boot_order` 默认/最小 2；local/logic verified，live 待验证，不标 production ready。
- KubeVirt VM lifecycle subresource：KUBEVIRT-VM-LIFECYCLE-SUBRESOURCE-A；VM start/stop 不再 PUT 主资源 `?start=true/stop=true`，改为调用 `subresources.kubevirt.io` 的 `/virtualmachines/{name}/start|stop` 并发送 `StartOptions`/`StopOptions`，同时对该 subresource 使用 `Accept: */*`，修复真实 KubeVirt 返回 `Kind missing in {}` 的 400 和 `406 Not Acceptable`；local/logic verified，live 待验证，不标 production ready。
...
**→ 继续入口：** 当前切片、验收命令、冻结目录见 [`repo/CURRENT-SPRINT.md`](repo/CURRENT-SPRINT.md)；破坏性磁盘操作、默认 StorageClass 切换、已有 PVC 迁移、HDD class 引入、并发重启或更大故障演练仍须单独审批。
...
> **v1.0.0 目标：** 2026-09-30 交付 ANI Core v1.0.0（当前未发布，不得标 v1.0.0/RC）；Services P0 由外部团队交付，不在本仓库开发。
