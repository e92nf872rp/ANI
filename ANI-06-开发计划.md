...
- ISO/CDI image upload CDI adapter + Gateway routes：IMAGE-UPLOAD-CDI-A（Task 3）；新增 `CDIImageImportService`/`CDIRESTClient`，`CreateUpload` 创建 DataVolume（`source.upload` + immediate-bind 注解 + `ani-rbd-ssd`）与 `UploadTokenRequest`，Get/List 把 DV phase 映射到 `ImageState`；Gateway `GET/POST/DELETE /api/v1/images*` 只调用 `ports.ImageImportService`，handler 不拼 CDI YAML；`IMAGE_IMPORT_PROVIDER=local|cdi_rest` 接线，`business-stack.yaml` 增加对应 env；SDK 已重生成含 Images 操作；fake REST 单测 + 假 HTTP transport 集成测试覆盖，沙箱无可达集群/`kubectl`，Step 4 现场冒烟未执行，不标 runtime/production ready，不实现 VM `boot_media`（留给 Task 4）。
- ISO/CDI VM boot_media=iso：VM-ISO-BOOT-A（Task 4）；`VMInstanceSpec` 增加 `BootMedia`/`BootMediaImageID`/`BootMediaBootOrder`，新增 `StorageAttachmentCDROM`；renderer 在 `boot_media=iso` 时渲染 `rootdisk`+`iso` 两个 `persistentVolumeClaim`（cdrom 带 `bootOrder`），不产出 `containerDisk`；planning 校验 `boot_image`/`boot_media` 互斥、`image_id`/`root_disk_size_gib` 必填，并在注入 `ImageImportService` 时校验 Image Ready；Gateway 解析 `boot_media`/`root_disk_size_gib` 并把 Task 3 已接线的 `ImageImportService` 传给 planner；既有 containerDisk 路径未改动；`boot_media.type=disk_image` 保持 `ErrUnsupported`；沙箱无可达集群/`kubectl`，Step 3 现场冒烟（noVNC 装机界面）未执行，不标 runtime/production ready。
