...
func TestTargetRegistryDecisionAndObligationCompleteness(t *testing.T) {
	...
	if got := len(registry.byRoute); got != 295 {
		t.Fatalf("frozen operation count = %d, want 295", got)
	}
	...
	if classifications[TargetAuthClassificationPublic] != 11 || classifications[TargetAuthClassificationAuthenticated] != 9 || classifications[TargetAuthClassificationAuthorized] != 275 {
		t.Fatalf("frozen auth classification counts = %#v", classifications)
	}
	if owners[TargetOwnerGateway] != 3 || owners[TargetOwnerCoreControl] != 205 || owners[TargetOwnerIAM] != 87 {
		t.Fatalf("frozen owner counts = %#v", owners)
	}
	if decisionCalls[0] != 11 || decisionCalls[1] != 284 || len(decisionCalls) != 2 {
		t.Fatalf("frozen IAM decision-call counts = %#v", decisionCalls)
	}
	if decisions[TargetIAMDecisionNone] != 11 || decisions[TargetIAMDecisionValidatePrincipal] != 9 || decisions[TargetIAMDecisionCheckPermission] != 275 || len(decisions) != 3 {
