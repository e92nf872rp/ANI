...
type processE2ETargetClient struct {
	delegate      targetiam.Client
	passwordCalls atomic.Int32
	decisionCalls atomic.Int32
}

func (c *processE2ETargetClient) PasswordLogin(ctx context.Context, request *iamv1.PasswordLoginRequest) (*iamv1.PasswordLoginResponse, error) {
	c.passwordCalls.Add(1)
	return c.delegate.PasswordLogin(ctx, request)
}

func (c *processE2ETargetClient) CheckPermission(ctx context.Context, request *iamv1.CheckPermissionRequest) (*iamv1.CheckPermissionResponse, error) {
...
func TestTargetIAMProcessE2EChild(t *testing.T) {
	...
	client, closeClient, err := targetiam.Dial(
	...
	unavailableClient, closeUnavailableClient, err := targetiam.Dial(unavailableAddress, processE2EMutualTLSConfig(t))
	if err != nil {
	...
	timeoutClient, closeTimeoutClient, err := targetiam.Dial(timeoutAddress, processE2EMutualTLSConfig(t))
	if err != nil {
...
func processE2EMutualTLSConfig(t *testing.T) targetiam.MutualTLSConfig {
	t.Helper()
	return targetiam.MutualTLSConfig{
...
func startProcessE2EGateway(t *testing.T, store middleware.GatewayStore, client targetiam.Client) string {
	t.Helper()
	address := reserveProcessE2ELoopbackAddress(t)
	h := server.New(server.WithHostPorts(address), server.WithExitWaitTime(1))
