...
def git_object_text(commit: str, path: str) -> str:
    try:
        ...
        raise ContractError(f"cannot read fixed Git object {commit}:{path}: {detail}") from error
...
def validate_replacement_manifest(
    ...
    try:
        ...
        raise ContractError(f"cannot resolve replacement source commit {commit}") from error
    if actual_tree != expected_tree:
        raise ContractError(f"replacement source tree is {actual_tree}, want {expected_tree}")
    ...
    for contract, field in (("core-v1", "core_openapi"), ("services-v1", "services_openapi")):
        ...
        text = git_object_text(commit, path)
