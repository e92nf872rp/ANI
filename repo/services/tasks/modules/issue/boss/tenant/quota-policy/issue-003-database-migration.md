...
创建数据库迁移文件 `20260810_002_tenant_plan_management.sql`：配额套餐表（tenant_plans + plan_quota_limits）+ tenants.plan_id 列 + audit_logs 月分区表。
...
## Dependencies
文件 1（`20260810_001_resource_quota.sql`）必须先执行（plan_quota_limits FK REFERENCES resource_quota_meta(resource_type) 依赖该表存在）
