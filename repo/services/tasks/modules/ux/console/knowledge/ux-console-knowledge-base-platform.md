...
2. 用户点击「新建知识库」按钮 → 弹出 Dialog（名称/描述/嵌入模型/chunk_size/top_k）
   → 填写并确认 → POST /api/v1/svc/knowledge-bases（含 idempotency_key）
...
| 知识库表格 | `Table` | columns: name/doc_count/status/created_at/actions; rowKey="id" | GET /api/v1/svc/knowledge-bases → items |
...
| 创建表单 | `Form` + `FormItem` | name(Input required) / description(Textarea) / embedding_model(Input default bge-m3) / chunk_size(InputNumber default 512) / top_k(InputNumber default 5) | POST /api/v1/svc/knowledge-bases body |
...
| State | Trigger | UI behavior | Components |
|-------|---------|-------------|------------|
| kb-not-found | GET /api/v1/svc/knowledge-bases/{kb_id} 404 | 整页 `Empty`「知识库不存在或已删除」 | Empty |
