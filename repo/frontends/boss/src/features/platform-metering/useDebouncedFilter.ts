/**
 * BOSS 平台计量 — debounce 筛选 hook + 辅助函数。
 *
 ...
 */
export function useDebouncedFilter(filter: PlatformUsageFilter, delayMs: number = DEFAULT_DEBOUNCE_MS): PlatformUsageFilter {
  const [debounced, setDebounced] = useState<PlatformUsageFilter>(filter)
