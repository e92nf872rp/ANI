...
describe('useDebouncedFilter', () => {
  ...
  it('延迟内再次变更应取消旧值，只返回最新值', () => {
    ...
    const filter1: PlatformUsageFilter = { ...baseFilter, resource_type: 'instance_gpu_seconds' }
    const filter2: PlatformUsageFilter = { ...baseFilter, resource_type: 'token_input' }
  ...
  it('支持自定义延迟', () => {
    ...
    const newFilter: PlatformUsageFilter = { ...baseFilter, group_by: 'day' }
    rerender({ filter: newFilter })
  ...
  it('tenant_id 变更也应 debounce', () => {
    const { result, rerender } = renderHook(({ filter }) => useDebouncedFilter(filter), {
      initialProps: { filter: baseFilter },
    })

    const drilldownFilter: PlatformUsageFilter = {
      ...baseFilter,
      tenant_id: 'tenant-456',
      group_by: 'day',
    }

    rerender({ filter: drilldownFilter })

    act(() => {
      vi.advanceTimersByTime(DEFAULT_DEBOUNCE_MS)
    })

    expect(result.current).toEqual(drilldownFilter)
  })
})
