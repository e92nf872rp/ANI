...
export interface paths {
    ...
    "/notifications/email/channel": {
        ...
        /**
         * 获取平台邮件发信通道
         * @description 返回平台级 SMTP 通道（单例）。敏感字段（password、auth_code）不回显明文，
         *     响应中只提供 has_password / has_auth_code 布尔位。未配置时返回 200 + state=disabled + has_password=false，
         ...
         *     password 与 auth_code 均为 write-only；留空或省略表示保留原值。
         */
    ...
    "/notifications/email/test-send": {
        ...
        /**
         * 发送测试邮件
         * @description 前置条件：通道已配置（has_password=true 或 has_auth_code=true + state=active）且至少有一个 enabled=true 的收件人。
