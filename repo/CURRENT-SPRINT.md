...
> **QUOTA-LIST-DISABLED-FILTER-A（2026-09-24，live verified，分支 hotfix/gpu-occupancy-scope，随 PR #185 评审合并）：** 修复用户报障 BOSS 配额列表 `GET /api/v1/quotas` 返回 48 条而【活跃】+【冻结】租户共 47 个的问题。根因两层：① `PostgresQuota.List` 分页第一步 SQL `SELECT DISTINCT tenant_id FROM resource_quota` 直接按配额表枚举租户，不 JOIN `tenants`、不过滤 `status`，禁用租户照常出现在列表；② `TenantService.DisableTenant` 是终态转换且注释明确「不释放资源」，`resource_quota` 行全部保留（清理能力 `DELETE /admin/tenants/{tenant_id}/quota` 为独立管理端点，禁用流程不自动触发）。修复：step1 SQL 改 `JOIN tenants` 并过滤 `t.status IN ('active', 'frozen')`（keyset cursor 仍原生 UUID 比较走索引）；指定 `tenant_id` 的单租户显式查询（`GetMy` 路径）不过滤、继续可用；**`resource_quota` 行保留，「禁用不释放资源」语义不变**；新增回归测试 `TestPostgresQuotaStoreListFiltersDisabledTenants`（断言 step1 SQL 必须含 `JOIN tenants` 与状态过滤）；`v1.yaml /quotas` description 同步声明新口径（仅 description 文字，无 schema/端点变更，非破坏性）。**live 验证抓到首版 SQL 缺陷并修复**：`ORDER BY rq.tenant_id`（uuid）与 `SELECT DISTINCT rq.tenant_id::text` 表达式不一致触发 PG `42P10`（本地 fake tx 测试不执行真 SQL 未拦住），修正为 `ORDER BY rq.tenant_id::text`。**live 验证 PASS（ani-test2 30083，并集镜像 `test2-20260924-quotafilter2`，含 GPU-OCCUPANCY-CARD-COUNT-A；原独立 PR #187 关闭，本批次随 PR #185 评审合并）**：`/admin/tenants` 真值 54（active 31/frozen 7/disabled 16），`/quotas` 分页 200 列出 29 租户、16 个禁用租户 0 泄漏（修复前 14 个有配额行的禁用租户全部混入）；DB 对账 `DISTINCT tenant_id=43 == 列表 29 + disabled 有行 14`，禁用租户配额行未删除；9 个 active/frozen 无配额行租户不列出属既有行为；回归 occupancy `physical=6/logical=24` 未回退。遗留：禁用租户历史配额只能按 tenant_id 显式查询（数据在库）；「列表带状态标注」如需属契约/前端独立批次。记录：`development-records/quota-list-disabled-filter-a.md`。
> **GPU-OCCUPANCY-CARD-COUNT-A（2026-09-24，live verified，分支 hotfix/gpu-occupancy-scope）：** 修复 BOSS 统计卡「物理卡/逻辑卡」数据错误（`GET /gpu-inventory/occupancy` 返回 `physical_card_count=24`/`logical_card_count=96`，真实为 **6/24**：3 节点 × 2 张物理 4090 × 每卡 4 切片）。根因：vGPU 节点的设备记录是**切片粒度**（每记录携带 `Shares=每卡切分数`），occupancy 统计按「每条记录即一张物理卡」——`PhysicalCardCount` 每记录 +1（=切片数）、`LogicalCardCount` 每记录 +Shares（=切片×切分数双重计数）；契约 description「与设备记录数同口径」固化了错误口径。修复：`GPUNodeClass` 新增 `PhysicalCards`（adapter 派生：vGPU 注解路径=注解段数、整卡=记录数、回退路径按 `gpu.count` 归组）；occupancy 物理卡优先用该字段、逻辑卡改为每记录 +1（=整卡+切片合计）；contract-first 修正 v1.yaml 两处 description（生成器不带属性描述，SDK/docs 零漂移）。响应 schema 形状零变更、无迁移。live 验证 PASS（ani-test2，镜像 `dev-20260924-gpu-card-count`）：`physical=6 / logical=24`，`total/in_use/available`（24/7/17）与 capacity `gpu_free=17` 不变；BOSS 30087 代理链路复核一致。记录：`development-records/gpu-occupancy-card-count-a.md`。

> **METERING-LIFECYCLE-EVENTS-A（2026-09-24，live verified，分支 hotfix/metering-gpu-lifecycle-events）：** 修复用户报障「新租户创建 GPU workload running 后 BOSS 计量 `instance_gpu_seconds=0`、`metering_usage_records` 无记录」。六层根因实测核实：① outboxWriter/metadataStore/storeTx 注入与 `GPU_QUOTA_ENABLED` 耦合（配额关闭即无生命周期事件）；② outbox payload 不符合 metering `InstanceLifecycleEvent` 契约；③ task-service publisher 按 `event.EventType` 作 NATS subject 与 metering 订阅的 `ani.events.instance.>` 不匹配（事件永不可达，新增发现）；④ payload 无 event_seq 致 consumer seenSeq 把后续事件全判过期丢弃（新增发现）；⑤ metering parseGPUCount 只认小写 `count` 而 `gpu_status` 落库为大写 `"Count"` → GPU 数解析 0（计量恒 0 直接死因）；⑥ metering Reconciler 只 StopStale 不补启动（另一死因）。修复跨三服务：gateway 解耦注入（controller/orchestrator/InstanceService 的 metadataStore/storeTx/outboxWriter 无条件、quota 仍门控）+ 完整契约 payload + 创建同步路径（confirmed/cancelled/retried/create_failed）与 API 生命周期路径（deleted/stopped/started）统一 outbox 写入（共享 `writeInstanceOutboxTx` helper）；task-service publisher 映射 subject `ani.events.instance.<uuid>` + 注入 `event_seq=outbox_events.id`；metering 兼容大写 Count + Reconciler 双向校准（幂等 StartCollection 补启动 + StopStale）。无契约/DB 迁移/生成物变更；多镜像部署（metering/task-service 因 NATS durable consumer 需 scale 0→1）。live E2E 两轮：创建 1×RTX 4090 → Reconciler 补启动 → `instance_gpu_seconds=60/period` 持续写入（原报障即修复）；删除 → outbox `instance.deleted`（完整契约 payload）→ 500ms 内发布 → metering 事件驱动 `collection stopped`（0.48 秒，不再等 5 分钟 tick），生产流量另两笔真实删除同样正常发布。单测 +5（lifecycle outbox）+ metering reconciler/spec 回归全绿；`make validate-architecture`/`gofmt`/`git diff --check` 通过。遗留：创建 confirmed 事件在「pod 慢启动+用户轮询详情」场景被详情接口 live 状态合成抢先落库绕过（架构缺口单独评估，metering 兜底不受影响）；metering kubelet_cpu/mem "no samples" 为既有环境问题。记录：`development-records/metering-lifecycle-events-a.md`。
> **GPU-OCCUPANCY-SCOPE-A（2026-09-23，live verified，分支 hotfix/gpu-occupancy-scope）：** 修复 `/api/v1/gpu-inventory/occupancy` 与 `/api/v1/platform/capacity` 空闲数互相矛盾（`available=24` vs `gpu_free=0`）。两接口同源同一 `GPUInventory` 与 `KubernetesRESTClient`，`total` 与 `gpu_total` 实测恒等（24），矛盾只在占用口径。三层根因：① occupancy 的 `in_use` 是租户级，平台 token 的 `tenant_id` 由 auth-service 置为 `uuid.Nil`、网关侧呈现为**全零 UUID**（30080 access log 实测），旧代码只判空串未识别全零 UUID，按该值查不存在的命名空间 `ani-tenant-00000000-…-0000` → 0 Pod → `in_use=0`；② 两侧都拿「带租户 label 的 Running Pod」当 GPU 占用、未校验 Pod 是否真的请求 GPU（偏离 `plan-platform-capacity.md` §3.4「Running 且请求 GPU 资源」口径），实测 53 个此类 Pod 只有 7 个真的请求 GPU，再按节点 `min(Pod 数, 设备数)` 截断使三节点顶满 → `in_use=24` → `gpu_free=0`；③ 30080 环境从未应用迁移 `20260911_001_gpu_device_surface.sql`（该批次只在 ani-test2 执行过），`gpu_device_overlays`/`gpu_device_events` 缺失致 `GET /gpu-inventory/events` 与 `PATCH /gpu-inventory/{device_id}` 报 `42P01`。修复：新增 `pkg/adapters/runtime/gpu_pod_occupancy.go` 作为唯一占用判定入口（`ParseRunningGPUPodOccupancy`：Running + `ani-tenant-*` + 请求 `nvidia.com/gpu`/`nvidia.com/vgpu`/`volcano.sh/vgpu-number`），平台容量与 gateway occupancy 共用；`gpuNodeOccupancy` 抽出 `gpuNodeOccupancyForRequest` + `platformScopeTenant`（识别空串与全零 UUID），平台占位值走集群级跨租户查询；聚合抽为 `gpuNodeOccupancyMapFromPods`。**契约零变更**（未改 v1.yaml、无生成物变更、无新增迁移）。live 验证 PASS（ani-system 10.10.1.66:30080，镜像 `dev-20260923-gpu-occupancy-scope2`）：occupancy `24/0/24` → **`total=24 in_use=7 available=17`**、capacity **`gpu_total=24 gpu_free=17`**（与集群真实值独立复算一致）；租户视角同为 7/17、租户访问 `/platform/capacity` 仍 403；迁移应用后两表 + RLS + 授权生效，events 转 200、PATCH maintenance → `available` 16 + `maintenance_count=1`、idle 还原 17/0。已知边界：`gpu_free` 按契约不扣台账（台账有 maintenance/unavailable 时与 `available` 差 `台账覆盖卡数 − 其中同时被 Pod 占用的卡数`，是否改属独立决策）；租户 scope 不合并台账；`logical_card_count` ≠ `gpu_total`。记录：`development-records/gpu-occupancy-scope-a.md`。
