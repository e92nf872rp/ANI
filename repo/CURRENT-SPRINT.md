...
| 实例交互式终端 | Browser WebSocket exec（2026-07-07） | `api/openapi/v1.yaml` / `demo_instances.go` / `pkg/ports/instance_observability.go` / `pkg/adapters/runtime/*instance_observability*` | **local/logic verified**（`INSTANCE-EXEC-WS-A`；`POST /instances/{id}/exec` 返回短期 token 与内嵌 token 的 `ws_url`，新增 `GET /instances/{id}/exec/{session}` WebSocket 握手入口，RBAC `scope:instances:exec`，local profile 支持原始 stdin/stdout 透传；不修改 Console 前端，不放宽鉴权；真实 Kubernetes Pod exec streaming 待 adapter 边界后续补齐） |
