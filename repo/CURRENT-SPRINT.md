...
> **PLATFORM-AUDIT-LOG（2026-09-10，K8s 测试环境实测，local 降级路径）：** BOSS 平台只读审计日志查询接口（分支 feat/platform-audit-log）：Core OpenAPI 契约优先新增 `GET /api/v1/platform/audit-logs`（time_from/time_to 必填成对，user/verb/resource_type/namespace/after/page_size/keyword 过滤，page_size 默认 20 上限 100 钳制；401/403/400 语义），数据源 kube-apiserver Metadata 级 write 审计经 fluent-bit 接流 Loki；ports `PlatformAuditService` + loki real adapter（query_range + LogQL label 过滤 + timestamp+auditID 全局倒序游标分页 + count_over_time 近似 total + Loki 不可用/非 200 单源降级 200 + real_provider=false）+ local 确定性降级（TotalApprox 随过滤裁剪）+ gateway 路由与 `AUDIT_LOG_PROVIDER`(local/loki) 装配 + authz/Core SDK 生成物零漂移（validate_gateway_authz_drift no drift）；修复 generate_gateway_authz.py Windows 写 CRLF 致生成全量漂移（write_text newline="\n"）；新增/增量单测全 PASS，平台/租户隔离红线（tenant→403）由 middleware 锁定。K8s 测试环境 ani-test2（镜像 test2-20260910-auditlog2）local 降级路径实测 T-3~T-5 全通过、T-4 边界 400、T-5 空窗 items:[]、T-6a/6b 401。真实控制面审计接流（apiserver static pod manifest 编辑在本环境被 kubelet 忽略）未启用 → real_provider=true 端到端未验证，属遗留风险；T-6c 租户 403 真实环境因无 tenant 登录端点回退单测锁定。详见 `development-records/platform-audit-log.md`。
