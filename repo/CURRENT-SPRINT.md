...
| Sandbox templates / runtime | Kata / runtimeClass / template catalog | `ports.SandboxTemplateCatalog` + `WorkloadRuntime` | **sandbox create live smoke passed**（`SANDBOX-KATA-WORKLOAD-RUNTIME-A`；Gateway `kind=sandbox` 在 `WORKLOAD_PROVIDER=kubernetes_rest` 下复用 workload orchestration，Deployment 使用 `runtimeClassName=sandbox-kata`，Pod Running/Ready 且 guest kernel 日志可见；evidence：`development-records/live-evidence/sandbox-kata-workload-runtime-live-evidence.json`；不代表 full platform production ready） |
...
| VM Console / noVNC | Browser WebSocket VNC proxy（2026-07-09） | `api/openapi/v1.yaml` / `demo_instances.go` / `pkg/ports/workload_runtime.go` / `pkg/adapters/runtime/kubernetes_instance_ops.go` / `kubernetes_console_stream.go` / `dryrun_renderer.go` | **local/logic verified**（`INSTANCE-CONSOLE-VNC-WS-A`；修复创建 session 时对 KubeVirt `/vnc` 的普通 HTTP GET 探测；`POST /instances/{id}/console` 返回内嵌 token 的 `connect_url`，新增 `GET /instances/{id}/console/{session}`；Gateway 以 `plain.kubevirt.io` 转发到 KubeVirt，供浏览器 noVNC 直连；`VM-NOVNC-USB-TABLET-A` 默认注入 KubeVirt USB tablet absolute pointer，减少 noVNC 鼠标坐标偏移；不改 Services/前端；不标 production ready） |
...
| ISO VM CD-ROM detach | 安装后从 ISO 引导切回系统盘（2026-07-10） | `instance_service.go` / `kubernetes_lifecycle_executor.go` / `dryrun_renderer.go` | **local/logic verified + live 待验证**（`ISO-CDI-VM-CDROM-DETACH-A`；VM `cdrom` 的 `detach_volume` 会调用 Kubernetes lifecycle provider，GET KubeVirt VM 后 merge-patch 删除 `iso` disk/volume；后续 bootOrder 语义由 `ISO-VM-BOOTORDER-DISK-FIRST-A` 收口；不标 production ready） |
| ISO VM bootOrder disk-first | 首启进 ISO、装完自重启进系统（2026-07-14） | `dryrun_renderer.go` / `demo_instances.go` / `api/openapi/v1.yaml` | **local/logic verified + live 待验证**（`ISO-VM-BOOTORDER-DISK-FIRST-A`；`rootdisk.bootOrder=1` + `iso.bootOrder=2`；`boot_media.boot_order` 默认/最小 2；不依赖删 pod/detach；不标 production ready） |
| KubeVirt VM lifecycle subresource | VM start/stop 真实 API 修复（2026-07-10） | `kubernetes_lifecycle_executor.go` | **local/logic verified + live 待验证**（`KUBEVIRT-VM-LIFECYCLE-SUBRESOURCE-A`；VM start/stop 改为 `subresources.kubevirt.io` `/virtualmachines/{name}/start|stop`，发送 `StartOptions`/`StopOptions` 且 `Accept: */*`，修复主资源 PUT `{}` 导致的 `Kind missing` 400 和 subresource `406 Not Acceptable`；不标 production ready） |
