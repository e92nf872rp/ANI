...
> **TENANT-BILLING-B（2026-09-08，local verified，Services PR）：** 租户计费操作历史（抽屉「操作历史」Tab）：追加第 6 个只读端点 `GET /api/v1/svc/billing/operations`（tenant_id/limit/offset，created_at 倒序，limit 上限 200 钳制，无足迹租户空列表不 404）+ 新表 `billing_operation_logs` 迁移（已在测试环境 PG 执行落表）；生成/结清/冲抵/调账 4 个写点与业务写**同事务**落流水，幂等重放与 409 冲突不落流水（混合联调实测锁定）；operator 透传 token user_id 经环境库复核。契约先行（services/v1.yaml + x-ani-authz read/platform）+ pb/Services SDK 四语言/docs/api 重生成幂等零漂移；顺带修复 `services/v1.yaml` 裸 `no` 被 YAML 1.1 解析为 False 的契约缺陷（加引号，JSON 字段名不变）。单测 svc +4 / gateway +2 全 PASS；混合联调 13 用例（e30–e43）全部通过（本地进程 × 环境 PG/Redis/auth-service，真实 401 + operator 环境库复核 + overview/CSV 全接口冒烟）；已合并 main（#148/#150/#152）并全量回归。Windows 本机 make 聚合入口环境性缺陷，批次内直跑底层校验脚本全绿，make 复核由人工补跑。未部署 K8s，不标 live/runtime ready。记录：`development-records/TENANT-BILLING-B.md`。
> **TENANT-BILLING-B 追加（2026-09-08）：账单软删除 `DELETE /api/v1/svc/billing/invoices/{invoiceId}`（第 7 个端点）。** 仅 issued 可删（终态 409 BILLING_STATE_CONFLICT；不存在/已删 404 幂等无害）；软删 `deleted_at` 标记行保留可审计，`(tenant_id, period)` 改部分唯一索引（唯一键释放、同账期可重新出账、单号 seq 不复用）；删除与 invoice.deleted 流水同事务，CAS 失败回滚不落流水；无幂等键（DELETE 非创建类写）。迁移 `20260908_002_billing_invoice_soft_delete.sql` 已应用测试环境 PG 并复核。顺带修复软删行对 settle/credit CAS 可见的边界缺陷（已删账单再结清 404，svc 断言锁定）。单测 svc +2 断言组 / gateway +1 全 PASS；混合联调 6/6 PASS（e44–e48，本地进程 × 环境中间件，预清理/复位 credit=104.6）。SDK/docs/pb 重生成幂等零漂移。

> **TENANT-BILLING-A（2026-09-08，local verified，Services PR）：** BOSS 租户计费结算 5 端点 `/api/v1/svc/billing/*`（总览/CSV 导出/生成账单/账单动作/调账）已按 `kjs-study/租户与计费用量/租户计费结算-后端接口方案.md` v0.3 落地：契约 `services/v1.yaml` + x-ani-authz（resource=billing，boundary=platform），实现 tenant-service gRPC（billing.proto + make gen-proto）+ 网关 mixed handler；生成即出账、一期一单幂等（同 key 重放返回原账单、换 key 409）、overdue 读取时动态计算不落库、余额读取推导、单价只读 `billing_pricing`（种子 ON CONFLICT DO NOTHING）、用量仅经 Core SDK 调 `/metering/usage/platform`、仅 token_total 计费。本地集成实测 21 用例通过（Docker PG + 双进程 + curl 矩阵）；测试环境被占用未部署 K8s，不标 live/runtime ready，真环境 RBAC 回归与成本对账留待后续批次。`make validate-services`/`make validate-architecture` EXIT:0；`make test` 中 validate-gateway-authz 的 operation-registry 漂移为 main 既有问题。记录：`development-records/TENANT-BILLING-A.md`。
