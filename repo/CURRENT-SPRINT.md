...
| Gateway 实例 workload 接线 | `WORKLOAD_PROVIDER=kubernetes_rest`（2026-06-30） | `workload_runtime.go` / `instance_workload_runtime.go` / `demo_instances.go` / `pkg/adapters/runtime` Secret+apply 修复 | **local/logic verified + 本机集群冒烟**（`GATEWAY-INSTANCE-WORKLOAD-RUNTIME-A`；gate：`validate-gateway-instance-workload-runtime`；对齐 M1-INSTANCE-P；**不新增 production-shaped live gate，不标 production ready**） |
| 实例网络选择 | Kube-OVN subnet annotation（2026-07-06） | `api/openapi/v1.yaml` / `demo_instances.go` / `dryrun_renderer.go` / `instance_store.go` | **local/logic verified + container live verified**（`INSTANCE-NETWORK-KUBEOVN-A`；`CreateInstanceRequest.network` 持久化到 `workload_instances`，Deployment/Job/KubeVirt template 注入 `ovn.kubernetes.io/logical_switch/ip_address`；真实 container Pod 在非重叠 `10.74.0.0/25` subnet 中 Running 且互 ping 通过；VM 为 manifest/template 单测覆盖，未标 VM live verified） |
| Console 网络管理后端契约 | `/api/v1/networks/*` Console gaps（2026-07-06） | `api/openapi/v1.yaml` / `network_resources.go` / `pkg/ports/network_resources.go` / `pkg/adapters/runtime/network_service.go` | **local/logic verified**（`CORE-NET-OPENAPI-CONSOLE-A`；补 SG rules 整包 PATCH、route GET/DELETE、subnet `vpc_id` 服务端筛选、VPC/subnet/SG/LB delete 409、subnet/LB/route create 404/409 校验；不新增 Services 路径，不修改 Console 前端） |
| 实例交互式终端 | Browser WebSocket exec（2026-07-07） | `api/openapi/v1.yaml` / `demo_instances.go` / `pkg/ports/instance_observability.go` / `pkg/adapters/runtime/*instance_observability*` | **local/logic verified**（`INSTANCE-EXEC-WS-A` + `INSTANCE-EXEC-KUBERNETES-STREAM-A`；`POST /instances/{id}/exec` 返回短期 token 与内嵌 token 的 `ws_url`，新增 `GET /instances/{id}/exec/{session}` WebSocket 握手入口，RBAC `scope:instances:exec`，local profile 使用 KubeCloud TerminalMessage JSON 帧；Prometheus/Kubernetes observability adapter 已补真实容器 exec stream：Gateway 委托 `InstanceExecSessionConnector`，Kubernetes `/pods/{pod}/exec` 使用 `v4.channel.k8s.io`，stdin/resize 映射 channel 0/4，stdout/stderr channel 1/2 映射回 `Op=stdout`；不放宽鉴权；集群暂不可连，live evidence 待补） |
| VM Console / noVNC | Browser WebSocket VNC proxy（2026-07-09） | `api/openapi/v1.yaml` / `demo_instances.go` / `pkg/ports/workload_runtime.go` / `pkg/adapters/runtime/kubernetes_instance_ops.go` / `kubernetes_console_stream.go` | **local/logic verified**（`INSTANCE-CONSOLE-VNC-WS-A`；修复创建 session 时对 KubeVirt `/vnc` 的普通 HTTP GET 探测；`POST /instances/{id}/console` 返回内嵌 token 的 `connect_url`，新增 `GET /instances/{id}/console/{session}`；Gateway 以 `plain.kubevirt.io` 转发到 KubeVirt，供浏览器 noVNC 直连；不改 Services/前端；不标 production ready） |
| ISO/CDI image upload foundation | CDI v1.65.0 foundation（2026-07-09） | `deploy/isolated/deploy.py` / `deploy/isolated/cluster-foundation/yaml/06-cdi-operator.yaml` / `07-cdi-cr.yaml` / `08-cdi-uploadproxy-nodeport.yaml` | **foundation deployment ready**（`CDI-FOUNDATION-A`；isolated foundation 顺序改为 KubeVirt → CDI → Rook → Harbor，operator manifest 中 CDI 组件镜像指向 `docker.changqingyun.cn/mirror/kubevirt/*:v1.65.0`，CDI CR 使用 `ani-rbd-ssd` scratch StorageClass，`cdi-uploadproxy-nodeport` 暴露 NodePort `31001`；仅为后续 ISO 直传提供底座，不实现 Images API/VM ISO boot，不标 production ready） |
| ISO/CDI image upload ports | Image import port + local adapter（2026-07-09） | `pkg/ports/image_import.go` / `pkg/adapters/runtime/local_image_import_service.go` / `pkg/bootstrap/deps.go` | **local/logic verified**（`IMAGE-UPLOAD-CDI-A`；`ImageImportService` 对齐 OpenAPI Image schemas，local profile 支持 `iso` 上传会话、幂等、Get/List/Delete；非 `iso` 返回 `ErrUnsupported`；不实现 CDI adapter、Gateway routes 或 VM ISO boot，不标 runtime/production ready） |
| ISO/CDI image upload CDI adapter + Gateway | CDI DataVolume/UploadTokenRequest REST adapter + Gateway `/images*`（2026-07-09） | `pkg/adapters/runtime/cdi_image_import.go` / `services/ani-gateway/internal/router/image_resources.go` / `services/ani-gateway/image_import_runtime.go` / `deploy/isolated/business-stack.yaml` | **local/logic verified，无 live 冒烟**（`IMAGE-UPLOAD-CDI-A` Task 3；`CDIImageImportService` 用 fake REST 单测覆盖 DataVolume `source.upload`+immediate-bind+`ani-rbd-ssd`、UploadTokenRequest、DV phase→`ImageState`、幂等重放；Gateway `GET/POST/DELETE /images*` 只调用 `ports.ImageImportService`；`IMAGE_IMPORT_PROVIDER=local\|cdi_rest` 接线并用假 HTTP transport 集成测试验证调用序列；`business-stack.yaml` 增加 `IMAGE_IMPORT_PROVIDER=cdi_rest`/`CDI_UPLOADPROXY_URL`；SDK 已重生成含 Images 操作；本沙箱环境无可达集群/`kubectl`，Step 4 现场冒烟未执行，不标 runtime/production ready；不实现 VM `boot_media`（Task 4）） |
| ISO/CDI VM boot_media=iso | KubeVirt VM CD-ROM boot + 空白系统盘（2026-07-09） | `pkg/ports/workload_runtime.go` / `pkg/adapters/runtime/dryrun_renderer.go` / `pkg/adapters/runtime/planning.go` / `services/ani-gateway/internal/router/demo_instances.go` | **local/logic verified，无 live 冒烟**（`VM-ISO-BOOT-A` Task 4；`VMInstanceSpec` 新增 `BootMedia`/`BootMediaImageID`/`BootMediaBootOrder`，`StorageAttachmentCDROM` 对齐既有 OpenAPI `cdrom` 枚举；renderer ISO 路径渲染 `rootdisk`+`iso` 两个 `persistentVolumeClaim`（`iso` 带 `bootOrder`），绝不产出 `containerDisk`；planning 校验 `boot_image`/`boot_media` 互斥与必填项，注入 `ImageImportService` 时额外校验 Image Ready（`ErrConflict`）；Gateway 解析 `boot_media`/`root_disk_size_gib` 并把已接线的 `IMAGE_IMPORT_PROVIDER` 传给 planner；既有 containerDisk 路径未改动；本沙箱环境无可达集群/`kubectl`，Step 3 现场冒烟（含 noVNC 装机界面）未执行，不标 runtime/production ready；`boot_media.type=disk_image` 保持 `ErrUnsupported`） |
