...
> **INFERENCE SERVICE ENGINE EXTRA ARGS CONTRACT C35（2026-08-19）：** `INFERENCE-SERVICE-ENGINE-EXTRA-ARGS-CONTRACT-C35` 已补齐 Services 创建契约可选 `engine.extra_args`：创建时冻结追加 CLI 参数，响应只读回显；保留名（model/host/port/served-model-name/TP/Ray backend/device）由后续实现返回 400；不进入 PATCH。不含 handler/proto/`engine.Launch`。无新 live，不得标记 GPU/runtime ready。
