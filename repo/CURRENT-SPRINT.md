...
> **INSTANCE-SANDBOX-STATELESS-A（2026-08-02）：** live passed。Core 使用请求级 PG 上下文、UUID、PG AsyncTaskStore、Redis DELETE/指纹/Token 过期幂等、checkpoint 422 和端口摘要写回；Gateway `instance-sandbox-stateless-20260802-v1` 真实 rollout 后，实例/文件/端口/task 恢复、幂等重放与冲突、Token 过期、生命周期及 PG/Kubernetes 清理全部通过。evidence：`development-records/live-evidence/instance-sandbox-stateless-live-20260802.json`。范围只覆盖 Gateway 重启；Sandbox Pod 重建仍会丢 `emptyDir` 文件，checkpoint real-provider 仍未实现。
