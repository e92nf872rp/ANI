...
1. `LocalInstanceService` 在配置了 `SandboxRuntime` 时保持 local sandbox profile；未配置 `SandboxRuntime` 时让 sandbox 走通用 workload orchestrator。
2. Gateway workload provider 为 `kubernetes_rest` 时不注入 local sandbox runtime，复用既有 dry-run / apply / status / store 链路。
