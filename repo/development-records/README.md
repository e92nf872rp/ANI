...
| services-controlled-unfreeze-a.md | SERVICES-CONTROLLED-UNFREEZE-A：当前治理入口从旧 Services 冻结规则切换为受控并行 PR；明确目录 ownership、CODEOWNERS 共同审查、API split、Services boundary gate、architecture gate，以及 Task 1 三项 accepted baseline violation 的非生产就绪边界 | Governance（Core/Services） |
