...
| INFERENCE-SERVICE-ENGINE-EXTRA-ARGS-CONTRACT-C35 | Services 创建推理服务补齐可选 `engine.extra_args`，创建时冻结、响应只读；保留名 400；不进入 PATCH。不含 handler/proto/Launch 实现。不得外推 runtime ready | inference-service-engine-extra-args-contract-c35.md |
