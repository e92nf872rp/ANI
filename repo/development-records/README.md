...
| SANDBOX-KATA-WORKLOAD-RUNTIME-A | Sandbox `kind=sandbox` 接入 Gateway Kubernetes workload provider：`WORKLOAD_PROVIDER=kubernetes_rest` 时复用既有 WorkloadRuntime 渲染/应用 Deployment，使用 `runtimeClassName=sandbox-kata`；补 `workload_kind='sandbox'` DB 约束迁移；真实 live smoke passed，脱敏 evidence 已归档；不标 full platform production ready | sandbox-kata-workload-runtime-a.md |
