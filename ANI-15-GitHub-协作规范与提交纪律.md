...
CI 对 AI Coding 的最低行为要求是：修改 `ai/` 下 Python 源码必须在同一 PR 提交测试；Go lint 和依赖扫描必须覆盖 `repo/go.work` 实际列出的全部 module；Services boundary/API/route 的新增问题和过期 baseline 必须失败。存量 baseline 只能作为有负责人、有原因的迁移债务告警，不能作为新问题的通行证。
